		...
		Citizen.Wait(1)
			if(Vdist(methlocation["x"],methlocation["y"],methlocation["z"], pos.x, pos.y, pos.z) < 10.0)then
				...
				if(Vdist(methlocation["x"],methlocation["y"],methlocation["z"], pos.x, pos.y, pos.z) < 5.0)then
					...
					local finished = exports["ucrp-taskbar"]:taskBar(60000,"Searching...")
...
RegisterNetEvent('ucrp-shops:exchangeGold')
AddEventHandler('ucrp-shops:exchangeGold', function()
	if exports["ucrp-inventory"]:hasEnoughOfItem("goldbar",70,true) then
		local finished = exports["ucrp-taskbar"]:taskBar(120000,"Trading Bars")
		if finished == 100 then
			if exports["ucrp-inventory"]:hasEnoughOfItem("goldbar",70,true) then
				TriggerServerEvent('ucrp-banking:addMoney', 35000) --- MintHavoc Change Banking
...
AddEventHandler('lockpickshit:general', function()
	local finished = exports["ucrp-taskbar"]:taskBar(60000,"What dis????")
	if (finished == 100) then
...
AddEventHandler("illegal:crafting", function()
	...
	local finished = exports['ucrp-taskbar']:taskBar(12000, 'Inspecting Table')
	if (finished == 100) then
...
-- 	local finished = exports['ucrp-taskbar']:taskBar(20000, 'Smelting')
-- 	if (finished == 100) then
...
RegisterNetEvent("ucrp-shops:redCounter")
AddEventHandler("ucrp-shops:redCounter", function()
    TriggerEvent("server-inventory-open", "1", "redcircle_counter");
end)

RegisterNetEvent("ucrp-shops:redCounter2")
AddEventHandler("ucrp-shops:redCounter2", function()
...
AddEventHandler("hunting:level1", function()
	if exports["ucrp-inventory"]:getQuantity("coyotepelt") >= 1 then
		playerAnim()
		local finished = exports["ucrp-taskbar"]:taskBar(4000,"Selling Coyotepelt",true,false,playerVeh)
		if finished == 100 then
			if exports["ucrp-inventory"]:getQuantity("coyotepelt") >= 1 then
				ClearPedTasksImmediately(PlayerPedId())
				TriggerEvent('inventory:removeItem', 'coyotepelt', 1)
				TriggerServerEvent('ucrp-banking:addMoney', math.random(80, 150))--- MintHavoc Change Banking
			else
...
AddEventHandler("hunting:level2", function()
	if exports["ucrp-inventory"]:getQuantity("buck") >= 1 then
		playerAnim()
		local finished = exports["ucrp-taskbar"]:taskBar(4000,"Selling Buck",true,false,playerVeh)
		if finished == 100 then
			if exports["ucrp-inventory"]:getQuantity("buck") >= 1 then
				ClearPedTasksImmediately(PlayerPedId())
				TriggerEvent('inventory:removeItem', 'buck', 1)
				TriggerServerEvent('ucrp-banking:addMoney', math.random(160, 200))--- MintHavoc Change Banking
			else
...
AddEventHandler("hunting:level3", function()
	if exports["ucrp-inventory"]:getQuantity("boar") >= 1 then
		playerAnim()
		local finished = exports["ucrp-taskbar"]:taskBar(4000,"Selling Boar",true,false,playerVeh)
		if finished == 100 then
			if exports["ucrp-inventory"]:getQuantity("boar") >= 1 then
				ClearPedTasksImmediately(PlayerPedId())
				TriggerEvent('inventory:removeItem', 'boar', 1)
				TriggerServerEvent('ucrp-banking:addMoney', math.random(350, 500))--- MintHavoc Change Banking
			else
...
--     exports['ucrp-menuinteract']:AddButton("Sell Coyotepelt", "Wow you suck", "hunting:level1", 'example button 1', "jobmenu")
--     exports['ucrp-menuinteract']:AddButton("Sell Buck", "Wow you kinda okay", "hunting:level2", 'example button 2', "jobmenu")
--     exports['ucrp-menuinteract']:AddButton("Sell Boar", "Wow you good", "hunting:level3", 'example button 3', "jobmenu")
--     exports['ucrp-menuinteract']:SubMenu("Hunting Menu" , "Click to select sell!" , "jobmenu")
-- end

--[[RegisterNetEvent("ucrp-scoreboard:playerscount")
AddEventHandler("ucrp-scoreboard:playerscount", function(a)
