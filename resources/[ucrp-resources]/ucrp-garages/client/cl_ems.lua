...
Citizen.CreateThread(function()
    exports["ucrp-polyzone"]:AddBoxZone("ems_buy", vector3(301.07, -574.73, 43.26), 1.4, 1, {
        name="ems_buy",
        heading=18,
        --debugPoly=true,
        minZ=40.46,
        ...
        while nearBuy2 do
            ...
            if job == 'ems' then
                if IsControlJustReleased(0, 38) then
                    TriggerEvent('ucrp-garages:openBuyMenu2')
                end
            end
        end
    end)
...
RegisterNetEvent('ucrp-garages:openBuyMenu2')
AddEventHandler('ucrp-garages:openBuyMenu2', function()
    TriggerEvent('ucrp-context:sendMenu', {
		{
			...
			params = {
				event = "ucrp-garages:PurchasedSpeedo"
			}
		},
        --[[{ WIP
			id = 2,
			...
			txt = "Purchase for $5000",
			params = {
				event = "ucrp-garages:PurchasedBamb"
			}
		},--]]
        {
			id = 2,
			...
			params = {
				event = "ucrp-garages:PurchasedBexp"
			}
		},
        {
			id = 3,
			...
			params = {
				event = "ucrp-garages:PurchasedEMSPRO"
			}
		},
...
RegisterNetEvent('ucrp-garages:PurchasedSpeedo')
AddEventHandler('ucrp-garages:PurchasedSpeedo', function()
    if exports["isPed"]:isPed("mycash") >= 5000 then
        --TriggerServerEvent('bankingremoveMoney')
        TriggerServerEvent('yorktax:speedo')
        TriggerEvent('ucrp-garages:PurchasedVeh2', 'Speedo', 'emsnspeedo', '0')
    else
        TriggerEvent('DoLongHudText', "You do not have enough money!", 2)
    end
end)

RegisterNetEvent('ucrp-garages:PurchasedBamb')
AddEventHandler('ucrp-garages:PurchasedBamb', function()
    if exports["isPed"]:isPed("mycash") >= 5000 then
        --TriggerServerEvent('bankingremoveMoney', 5000)
        TriggerServerEvent('yorktax:bamb')
        TriggerEvent('ucrp-garages:PurchasedVeh2', 'Ambulance', 'bamb', '0')
    else
        TriggerEvent('DoLongHudText', "You do not have enough money!", 2)
    end
end)

RegisterNetEvent('ucrp-garages:PurchasedBexp')
AddEventHandler('ucrp-garages:PurchasedBexp', function()
    if exports["isPed"]:isPed("mycash") >= 15000 then
        --TriggerServerEvent('bankingremoveMoney', 5000)
        TriggerServerEvent('yorktax:bexp')
        TriggerEvent('ucrp-garages:PurchasedVeh2', 'Explorer', 'bexp', '0')
    else
        TriggerEvent('DoLongHudText', "You do not have enough money!", 2)
    end
end)   

RegisterNetEvent('ucrp-garages:PurchasedEMSPRO')
AddEventHandler('ucrp-garages:PurchasedEMSPRO', function()
    if exports["isPed"]:isPed("mycash") >= 35000 then
        --TriggerServerEvent('bankingremoveMoney', 5000)
        TriggerServerEvent('yorktax:emspro')
        TriggerEvent('ucrp-garages:PurchasedVeh2', 'Sierra', 'emspro', '0')
    else
        TriggerEvent('DoLongHudText', "You do not have enough money!", 2)
    end
end)  

RegisterNetEvent('ucrp-garages:PurchasedVeh2')
AddEventHandler('ucrp-garages:PurchasedVeh2', function(name, veh, price)
    ...
    TriggerServerEvent('ucrp-garages:FinalizedPur2', plate, name, vehicle, price)
    Citizen.Wait(100)
    exports['ucrp-ui']:hideInteraction()
end)
