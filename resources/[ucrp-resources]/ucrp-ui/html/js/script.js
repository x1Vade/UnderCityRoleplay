...
function correct(){
    ...
    $.post('https://ucrp-ui/ucrp-ui:taskBarSkillResult', JSON.stringify({
        success: true,
    }));
}
...
const CloseMenu = () => {
    ...
    saved = "";
    textboxactive = false
    $.post(`https://ucrp-ui/guccimanecancel`)
...
const PostData = (data) => {
    return $.post(`https://ucrp-ui/`+textmenucallbackurl, JSON.stringify(data))
}

function closesettings(){
    $(".settings").hide();
    $.post('https://ucrp-ui/close');
}
    ...
    switch (action) {
        ...
        case "eye":
            ...
            if (msg.data.action == "refresh") {
                ...
                $.post('https://ucrp-ui/closecontext', JSON.stringify({}));
            }
            break;
        case "sniper-scope":
...
$(document).on('mousedown', (event) => {
    ...
    if (element.id.split("-")[0] === 'target') {
        ...
        $.post('https://ucrp-ui/ucrp-ui:targetSelectOption', JSON.stringify({
            entity : entitydata,
            option :{
                event: eventData,
...
$(document).keydown(function(event) {
    ...
    if (key == 27) {
        ...
        if (eyeintactive){
            eyeintactive = false
            $.post('https://ucrp-ui/closeeyeint', JSON.stringify({}));
        }
    }
});
        ...
        if (event.target.id == "backmenu") {
            ...
            if (event.target.id.split("-")[0] === 'contextmenu') {
                ...
                if (children) {
                    ...
                    $(".buttoncontext").html("");
                    $(".containercontext").hide();
                    $.post('https://ucrp-ui/'+action, JSON.stringify({
                        key:key
                    }));
                ...
                $(".buttoncontext").html("");
                $(".containercontext").hide();
                $.post('https://ucrp-ui/'+subaction, JSON.stringify({
                    key:subkey
                }));
            }
        }
