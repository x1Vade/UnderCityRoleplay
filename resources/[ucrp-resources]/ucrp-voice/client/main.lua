...
AddEventHandler('ucrp-voice:settingsCallback', function(cb)
	cb(Cfg)
end)

RegisterNetEvent('ucrp-voice:SetRadioVolume')
AddEventHandler('ucrp-voice:SetRadioVolume', function(passedRadioVolume)
	volume = passedRadioVolume
end)

RegisterNetEvent('ucrp-voice:updateRoutingBucket')
AddEventHandler('ucrp-voice:updateRoutingBucket', function(routingBucket)
...
RegisterCommand('+cycleproximity', function()
	...
	TriggerEvent('ucrp-voice:setTalkingMode', voiceMode)
	TriggerEvent("ucrp-hud:changeRange", voiceMode)
end, false)
RegisterCommand('-cycleproximity', function()
end)
...
RegisterNetEvent('ucrp-voice:mutePlayer')
AddEventHandler('ucrp-voice:mutePlayer', function()
...
function setVoiceProperty(type, value)
	...
	elseif type == "micClicks" then
		...
		micClicks = val
		SetResourceKvp('ucrp-voice_enableMicClicks', val)
	end
end
...
RegisterCommand('vsync', function()
	local newGrid = getGridZone()
	debug(('[ucrp-voice] [vsync] Forcing zone from %s to %s and adding nearby grids.'):format(currentGrid, newGrid))
...
AddEventHandler('onClientResourceStart', function(resource)
	...
	local micClicksKvp = GetResourceKvpString('ucrp-voice_enableMicClicks')
	if not micClicksKvp then
		SetResourceKvp('ucrp-voice_enableMicClicks', tostring(true))
	else
	...
	print('[ucrp-voice] Intitalized voices.')
	intialized = true
...
RegisterCommand("grid", function()
	print(('[ucrp-voice] Players current grid is %s'):format(currentGrid))
end)