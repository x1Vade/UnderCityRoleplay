...
function NPX.Jobs.CanBecomeJob(self, user, job, callback)
    ...
    if not hexId or not characterId or not src then callback(false, "Id's don't exist") return end
        ...
        TriggerEvent("ucrp-jobmanager:attemptBecomeJob", src, characterId, function(allowed, reason)
...
function NPX.Jobs.SetJob(self, user, job, notify, callback)
    ...
    NPX.Jobs:CanBecomeJob(user, job, function(allowed, reason, rank) 
        ...
        TriggerClientEvent("ucrp-jobmanager:playerBecameJob", src, job, name, false)
        TriggerClientEvent("ucrp-jobmanager:playerBecomeEvent", src, job, name, notify)
...
AddEventHandler("ucrp-base:exportsReady", function()
    exports["ucrp-base"]:addModule("JobManager", NPX.Jobs)
end)
                ...
                for src,data in pairs(tbl) do
                    ...
                    if user then
                        ...
                        if tonumber(curTime) == tonumber(data.lastPayCheck) or tonumber(data.lastPayCheck) >= 480 then
                            ...
                            --exports["ucrp-log"]:AddLog("Job Pay", user, "User recieved paycheck, amount: " .. tostring(payCheck))
                        else
...
RPC.register("ucrp-jobmanager:jobCount", function(pSource, pJob)
