    ...
    if police >= 0 then
        if exports["ucrp-inventory"]:hasEnoughOfItem("safecrackingkit", 1) then
            if store_crack_1 == true then
                TriggerEvent("ucrp-dispatch:storerobbery")
...
AddEventHandler('dark-storerob:stores:store_openSafe_21', function()
    if store_getmoney_21 == true then
        local random = math.random(24, 42)
        local markedbills = math.random(3, 6)
        ClearPedTasks(PlayerPedId())
           TriggerEvent('player:receiveItem', 'rollcash', random)
           TriggerEvent('player:receiveItem', 'bands', markedbills) 
           ClearPedTasks(PlayerPedId())
...
RegisterNetEvent("ucrp-heists:safe:rob", function()
    ...
    if policecount >= 1 then
        if GetEntityHealth(obj) < 800 then
            local finished = exports["ucrp-ui"]:taskBarSkill(25000, math.random(10, 20))
            if (finished == 100) then
                TriggerEvent("ucrp-dispatch:storerobbery")
                local finished = exports["ucrp-ui"]:taskBarSkill(30000, math.random(10, 20))
                if (finished == 100) then
                    TriggerEvent("ucrp-dispatch:storerobbery")
                    ClearPedTasks(PlayerPedId())
