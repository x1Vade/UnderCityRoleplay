...
--   local balance = RPC.execute("ucrp-casino:getCurrentChipCount")
--  --local balance = 400
--   local values = {}
    if state then 
...
--     -- exports["ucrp-ui"]:sendAppEvent("status-hud", {
--     --   show = state,
--     --   title = title,
--     --   values = values
--     -- })
--     exports["ucrp-textdisplay"]:DisplayText({
--       header = title,
--         rows = {
--           {
--               id = 0, 
...
--   })
    ...
    exports["ucrp-textdisplay"]:HideText()
  end
end
...
local function getUpFromSlotMachine()
  ...
  TriggerServerEvent("ucrp-slotmachines-sv:getUpFromSlotMachine")
  drawStatusHUD(false, {})
    ...
    if isSeatedAtSlotMachine then
      if not isBetContextShowing then
        if currentSlotMachineDetails ~= nil then
          ...
          table.insert(data, {
            action = "ucrp-ui-slotmachines:placeBet",
            title = "Place bet",
            description = "$100.00",
            key = {
              coinValue = 100,
          ...
          table.insert(data, {
            action = "ucrp-ui-slotmachines:leaveMachine",
            title = "Leave Machine",
            key = {},
          })
...
local function initializeSlotMachines()
  ...
  TriggerServerEvent("ucrp-slotmachines-sv:getMachineData")
        ...
        while true do
          ...
          if not isNear(plyPos, v.chairPos, 0.75) then
            if isInteractionShowing then
              ...
              exports["ucrp-ui"]:hideInteraction(interactionBackground)
            end
          ...
          if not isInteractionShowing then
            ...
            if exports["ucrp-inventory"]:hasEnoughOfItem("casinomember", 1, false, true) then
              interactionBackground = "success"
              canPlayAtSlotMachine = true
            elseif exports["ucrp-inventory"]:hasEnoughOfItem("casinovip", 1, false, true) then
              interactionBackground = "success"
              canPlayAtSlotMachine = true
            else
            ...
            exports["ucrp-ui"]:showInteraction("[E] Play " .. v.label, interactionBackground)
          end
        
          if IsControlJustReleased(1, 38) and canPlayAtSlotMachine then --Key: E
            TriggerServerEvent("ucrp-slotmachines-sv:sitDownAtSlotMachine", k)
            --print("E")
            break
          end
...
--#[Event Handlers]#--
RegisterNetEvent("ucrp-slotmachines-cl:getMachineData")
AddEventHandler("ucrp-slotmachines-cl:getMachineData", function(data)
...
RegisterNetEvent("ucrp-slotmachines-cl:updateMachineData")
AddEventHandler("ucrp-slotmachines-cl:updateMachineData", function(machine, ply)
...
RegisterNetEvent("ucrp-slotmachines-cl:sitDownAtSlotMachine")
AddEventHandler("ucrp-slotmachines-cl:sitDownAtSlotMachine", function(machine, coinValue)
  local plyPed = PlayerPedId()
  local plyPos = GetEntityCoords(plyPed)
...
RegisterNetEvent("ucrp-slotmachines-cl:playSlotMachineSound")
AddEventHandler("ucrp-slotmachines-cl:playSlotMachineSound", function(machinePos, machineModel, sound)
  if isSMSetup then
    local soundRef = ""
...
RegisterNetEvent("ucrp-slotmachines-cl:stopReels")
AddEventHandler("ucrp-slotmachines-cl:stopReels", function(reel, rot)
...
RegisterNetEvent("ucrp-slotmachines-cl:spinFinished")
AddEventHandler("ucrp-slotmachines-cl:spinFinished", function(amountWon)
  if amountWon > 0 then
    -- RPC.execute("ucrp-casino:winChips", amountWon)
...
RegisterNetEvent("ucrp-slotmachines-cl:toggleSlotMachines")
AddEventHandler("ucrp-slotmachines-cl:toggleSlotMachines", function(state)
...
AddEventHandler("ucrp-casino:casinoEnteredEvent", function()
  isInsideCasino = true
...
AddEventHandler("ucrp-casino:casinoExitedEvent", function()
...
--#[NUI Callbacks]#--
RegisterUICallback("ucrp-ui-slotmachines:placeBet", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  exports["ucrp-ui"]:hideContextMenu(data)
  ...
  if currentSlotMachine and not isSlotMachineSpinning then
    ...
    if not exports["ucrp-inventory"]:hasEnoughOfItem("redchip", currentBet, false, true) then 
    ...
    TriggerServerEvent(
      "ucrp-slotmachines-sv:spinMachine",
    ...
    local title = "Slot Machine - " .. currentSlotMachineLabel
    --local balance = RPC.execute("ucrp-casino:getCurrentChipCount")
    local balance = exports["ucrp-inventory"]:getQuantity("redchip")
    --local values = {}
    --table.insert(values, "Balance: $" .. balance)
      exports["ucrp-textdisplay"]:DisplayText({
      header = title,
...
RegisterUICallback("ucrp-ui-slotmachines:leaveMachine", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  exports["ucrp-ui"]:hideContextMenu(data)
...
function spinSlot(tickRate)
  ...
  for i=1,300,1 do
    ...
    elseif i == 180 then
      ...
      TriggerEvent("ucrp-slotmachines-cl:stopReels", 1, ang1)
    end
    if i < 240 then
    elseif i == 240 then
      ang2 = tickRate.b*22.5-180+0.0
      print(ang2)
      TriggerEvent("ucrp-slotmachines-cl:stopReels", 2, ang2)
    end
    if i < 300 then
    elseif i == 300 then
      ang3 = tickRate.c*22.5-180+0.0
      TriggerEvent("ucrp-slotmachines-cl:stopReels", 3, ang3)
    end				
    Citizen.Wait(10)
  end
  TriggerServerEvent('ucrp-slotmachines-sv:slotsCheckWin', tickRate, data)
end

RegisterNetEvent('ucrp-slotmachines-cl:startSpin')
AddEventHandler('ucrp-slotmachines-cl:startSpin',function(tickRate)
  spinSlot(tickRate)
end)
