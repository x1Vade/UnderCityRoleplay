...
AddEventHandler('spawn:clientSpawnData', function(spawnData, heheData)
		...
		if spawnData.overwrites ~= nil then
			...
			elseif spawnData.overwrites == "new" then
				...
				-- TriggerEvent("ucrp-spawn:firstspawnyo")
			end
			return
		end
		...
		--[[if Spawn.housingCoords == nil then
			Spawn.housingCoords = exports["ucrp-housing"]:retriveHousingTable();
		end]]
...
RegisterNetEvent("ucrp-spawn:firstspawnyo")
AddEventHandler("ucrp-spawn:firstspawnyo", function()
    if GetEntityModel(PlayerPedId()) == GetHashKey("mp_f_freemode_01") then
        TriggerEvent("ucrp-spawn:finishedClothingstuffbish")
    else
        TriggerEvent("ucrp-spawn:finishedClothingstuff")
    end
end)

RegisterNetEvent('ucrp-spawn:finishedClothingstuff')
AddEventHandler('ucrp-spawn:finishedClothingstuff', function()
...
RegisterNetEvent('ucrp-spawn:finishedClothingstuffbish')
AddEventHandler('ucrp-spawn:finishedClothingstuffbish', function()
...
function Spawn.selectedSpawn(spawnInfo)
	...
	if apartment then
		...
		if pos then
			...
			DoScreenFadeIn(2500)
			TriggerEvent("ucrp-spawn:characterSpawned")
			TriggerEvent("ucrp-spawn:characterSpawned")
		else 
