...
function Login.CreatePlayerCharacterPeds(characterModelData,isReset)
    ...
    local events = exports["ucrp-base"]:getModule("Events")
    events:Trigger("ucrp-base:fetchPlayerCharacters", nil, function(data)
        if data.err then
            return
        end
...
--[[
    ...
    Description: dealing with ucrp-base functionality in order to set/get the correct information for chars
]]
...
function Login.getCharacters(isReset)
    ...
    if not isReset then
        ...
        events:Trigger("ucrp-base:loginPlayer", nil, function(data)
...
function Login.SelectedChar(data)
	...
	local events = exports["ucrp-base"]:getModule("Events")
	events:Trigger("ucrp-base:selectCharacter", data.actionData, function(returnData)
        ...
        if Login.CurrentClothing[data.actionData] == nil then
	       ...
	       -- SetPlayerInvincible(PlayerPedId(), true)
	        TriggerEvent("ucrp-base:firstSpawn")
	    end
    end)
end
...
RegisterNetEvent("ucrp-spawn:finishedClothing")
AddEventHandler("ucrp-spawn:finishedClothing", function(endType)
    ...
    if distance <= 10 then
    	...
    	if endType == "Finished" then
            DoScreenFadeOut(2)
    		spawnChar()
            TriggerServerEvent("ucrp-spawn:licenses")
    	else
    		backToMenu()
    	end
    end	
...
function spawnChar()
    Login.actionsBlocked = false
    deleteTrain()
    TriggerEvent("ucrp-weathersync:spawned")
    -- SetPlayerInvincible(PlayerPedId(), true)
    TriggerEvent("ucrp-base:firstSpawn")
