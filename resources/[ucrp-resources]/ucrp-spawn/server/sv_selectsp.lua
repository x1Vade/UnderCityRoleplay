...
AddEventHandler('playerDropped', function(reason)
    ...
    if string.find(reason, "crash") or string.find(reason, "timed out") or string.find(reason, "Exiting") or
        string.find(reason, "Disconnected") then
...
AddEventHandler("character:loadspawns", function()
    ...
    exports.ghmattimysql:execute(
        "SELECT ho.*, cm.cid, cm.building_type, hp.level, hp.illness, hp.time FROM character_motel cm LEFT JOIN __housedata ho on ho.cid = cm.cid LEFT JOIN hospital_patients hp on hp.cid = cm.cid WHERE cm.cid = @cid ",
        {
            ["@cid"] = cid
            ...
            exports.ghmattimysql:execute('SELECT * FROM __housedata WHERE `cid` = @id', {
                ...
                if housingMotels[1] then
                    ...
                    exports.ghmattimysql:execute(
                        "INSERT INTO hospital_patients (cid,level,illness,time) VALUES (@cid,@level,@illness,@time)", {
                            ["cid"] = cid,
                            ["level"] = 0,
                            ["illness"] = "none",
                            ["time"] = 0
                    ...
                    exports.ghmattimysql:execute([[UPDATE characters SET new = 0 WHERE id = @cid AND new = 1;]], {
                        ["cid"] = char.id
                    }, function()
                        -- Create a new bank account for the character
                        exports["ucrp-financials"]:BankAccountCreation(cid, "Personal Account", 1, 5000, 0, 0, 0, 1, false)
                        print(("[Spriest] Created new bank account for %s(CID)"):format(cid))
                    end)
                    TriggerClientEvent("spawn:clientSpawnData", src, spawnData)
                end
            end)
