  ...
  while true do
    ...
    if creatingMap then
      ...
      if #currentMap == 0 then
        exports['ucrp-interaction']:showInteraction('[E] to add start point, up/down for size, phone to save or cancel.')
      else
        exports['ucrp-interaction']:showInteraction('[E] to add start point, up/down for size, phone to save or cancel.')
      end
...
if dist > 40.0 then
    TriggerEvent('DoLongHudText', 'You\'re too far away from the starting line.')
    EndRace()
...
RegisterNUICallback('racing:events:list', function()
  -- local isEmployed = exports["ucrp-business"]:IsEmployedAt("vanilla_unicorn")
  -- if exports["ucrp-inventory"]:hasEnoughOfItem("racingusb",1,false) then
  SendNUIMessage({
