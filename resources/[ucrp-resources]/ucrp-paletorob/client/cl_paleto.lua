...
AddEventHandler("ucrp-paleto:freezeDoors", function()
    Citizen.CreateThread(function()
...
-- RegisterNetEvent("ucrp-paleto:toggleVault")
...
--             TriggerServerEvent("ucrp-paleto:updateVaultState", key, Doors[key][1].state)
--         elseif state then
--             local obj = GetClosestObjectOfType(Paleto.Banks[key].doors.startloc.x, Paleto.Banks[key].doors.startloc.y, Paleto.Banks[key].doors.startloc.z, 2.0, GetHashKey(Paleto.vaultdoor), false, false, false)
--             local count = 0
...
AddEventHandler("ucrp-paleto:reset", function(name, data)
    ...
    -- TriggerServerEvent("ucrp-paleto:toggleVault", name, true)
...
AddEventHandler("ucrp-paleto:startheist", function(data, name)
  ...
  --  currentcoords = vector3(data.doors.startloc.x, data.doors.startloc.y, data.doors.startloc.z)
    ...
    --TriggerServerEvent("ucrp-paleto:toggleVault", name, false)
    startdstcheck = true
    currentname = name
...
function SpawnTrolleys(data, name)
    ...
    TriggerServerEvent("ucrp-paleto:startLoot", data, name, missionplayers)
    done = false
end
...
RegisterNetEvent('ucrp-paleto:darkmail')
AddEventHandler('ucrp-paleto:darkmail', function()
    Citizen.Wait(1000)
...
RegisterNetEvent('ucrp-paleto:UseBlueLapTop')
AddEventHandler('ucrp-paleto:UseBlueLapTop', function(item)
    local ply = PlayerPedId()
    local plyCoords = GetEntityCoords(ply)
    local p = promise:new()

    Paleto.Banks, doors = RPC.execute("ucrp-paleto:getBanks")
	TriggerEvent("ucrp-paleto:freezeDoors")
    ...
    for k, v in pairs(Paleto.Banks) do
        if not v.onaction then
            ...
            if dist < 1.0 then
                ...
                if exports["ucrp-inventory"]:hasEnoughOfItem("heistlaptop2", 1) then
                    -- if HasItem then
                    ...
                    exports['hacking']:OpenHackingGame(function(Success)
                        if Success then
                            ...
                            TriggerEvent("client:newStress",true,math.random(6,12))
...
AddEventHandler('paletogate:doorburn', function()
	TriggerServerEvent('ucrp-doors:change-lock-state', 516, false)
end)
