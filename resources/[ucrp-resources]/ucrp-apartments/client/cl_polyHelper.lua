...
function polyHelperEnter(apartmentLocNumber,apartmentNumber)
    ...
    listening = true
    exports["ucrp-ui"]:showInteraction(getInteractionMessage(apartmentLocNumber,apartmentNumber))
...
local function showApartmentMenu(apartmentLocNumber,apartmentNumber)
    ...
    for k,v in pairs(Apart.currentRoomLocks[apartmentLocNumber]) do
        if v == false then
            unlockedApartments[#unlockedApartments + 1] = { title = "Apartment # "..k, action = "ucrp-ui:apartmentsContext", key = {"EnterUnlocked",apartmentLocNumber,k} }
        end 
    end
...
RegisterUICallback("ucrp-ui:apartmentsContext", function(data, cb)
    ...
    if action == "locks" then
        ...
        exports["ucrp-ui"]:showInteraction(getInteractionMessage(globalLocNumber,globalNumber))
    end
    ...
    if action == "lockdownCid" then
        ...
        local prompt = exports['ucrp-ui']:OpenInputMenu(elements, function(values)
