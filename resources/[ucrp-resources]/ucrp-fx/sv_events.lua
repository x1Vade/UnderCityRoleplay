...
RegisterServerEvent("ucrp-fx:setSteamId")
AddEventHandler("ucrp-fx:setSteamId", function()
    local src = source
    local user = exports["ucrp-base"]:getModule("Player"):GetUser(src)
    local steamId = user:getVar("steamid")
    TriggerClientEvent("ucrp-fix:setSteamIdClient", src, steamId)
end)

RegisterServerEvent("ucrp-fx:smoke:grenade")
AddEventHandler("ucrp-fx:smoke:grenade", function(pCoords)
...
RegisterNetEvent("ucrp-fx:chain:blingDiamonds", function(pCoords, pType, pSize, pStrength, pScale)
