...
local function addToComboZone(zone)
    if comboZone ~= nil then
        ...
        comboZone = ComboZone:Create({ zone }, { name = 'ucrp-polyzone' })
        comboZone:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
            if leftZones ~= nil then
              for i = 1, #leftZones do
                TriggerEvent('ucrp-polyzone:exit', leftZones[i].name)
              end
            end
            if enteredZones ~= nil then
              for i = 1, #enteredZones do
                TriggerEvent('ucrp-polyzone:enter', enteredZones[i].name, enteredZones[i].data, enteredZones[i].center)
              end
            end
        end, 500)
    end
...
RegisterNetEvent('ucrp-polyzone:createCircleZone')
AddEventHandler('ucrp-polyzone:createCircleZone', function(name, ...)
  addCircleZone(name, ...)
end)
