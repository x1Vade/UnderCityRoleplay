        ...
        if inZone > 0 then
            ...
            placeObject("np_beehive", function(pPlaced, pCoords, pHeading)
                if pPlaced then
                    ...
                    local finished = exports["ucrp-taskbar"]:taskBar(15000, "Installing beehive", false, true, false, false, nil, 5.0, PlayerPedId())
                    ClearPedTasks(PlayerPedId())
                    if finished == 100 then
                        RPC.execute("ucrp-beekeeping:installHive", pCoords, pHeading)
...
AddEventHandler("ucrp-beekeeping:checkBeehive", function(pContext, pEntity)
...
RegisterUICallback('ucrp-beekeeping:addQueen', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_GARDENER_PLANT", 0, true)
    local finished = exports["ucrp-taskbar"]:taskBar(15000, "Adding Queen bee", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        RPC.execute("ucrp-beekeeping:addQueen", data.key.id)
        TriggerEvent("inventory:removeItem", "beequeen", 1)
    end
end)

RegisterUICallback('ucrp-beekeeping:removeHive', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent("animation:PlayAnimation","layspike")
    local finished = exports["ucrp-taskbar"]:taskBar(15000, "Removing", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        local getReady = getHiveReadyPercent(getHiveById(data.key.id)) > 20.0
        local success = RPC.execute("ucrp-beekeeping:removeHive", data.key, getReady)
        if not success then
...
AddEventHandler("ucrp-beekeeping:harvestHive", function(pContext, pEntity)
    local hiveId = getHiveId(pEntity)
    if not hiveId then return end
    ...
    TriggerEvent("animation:PlayAnimation","layspike")
    local finished = exports["ucrp-taskbar"]:taskBar(5000, "Harvesting", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        RPC.execute("ucrp-beekeeping:harvestHive", hiveId)
    end
end)

AddEventHandler("ucrp-polyzone:enter", function(zone, data)
    if zone == "ucrp-beekeeping:bee_zone" then
        inZone = inZone + 1
        if inZone == 1 then
            local result = RPC.execute("ucrp-beekeeping:getBeehives")
        end
    end
end)
...
RegisterNetEvent('ucrp-beekeeping:trigger_zone')
AddEventHandler('ucrp-beekeeping:trigger_zone', function (type, pData)
    --Update all hives
    if type == 1 then
...
function showHiveMenu(pHiveId)
    ...
    if getStageFromPercent(harvest) < 2 and not hive.has_queen then
        context[#context+1] = {
            title = 'Add Queen bee',
            key = { id = pHiveId },
            action = 'ucrp-beekeeping:addQueen',
            description = 'Make the hive happy.',
            disabled = not exports["ucrp-inventory"]:hasEnoughOfItem("beequeen", 1, false)
        }
    end
    ...
    if harvest >= 95 or myjob == "police" or myjob == "judge" then
        context[#context+1] = {
            title = 'Destroy Hive',
            key = { id = pHiveId },
            action = 'ucrp-beekeeping:removeHive',
        }
    end
