...
RegisterNetEvent('ucrp-items:SetAmmo')
AddEventHandler('ucrp-items:SetAmmo', function(sentammoTable)
...
function attemptToDegWeapon()
	if math.random(100) > 85 then
		...
		if  hasTimer >= 2000 then
			...
			TriggerEvent("inventory:DegenLastUsedItem",0.4)
		end
	end
end
...
function actionBarDown()
  ...
  TriggerServerEvent("ucrp-financials:cash:get", GetPlayerServerId(PlayerId()))
end
...
Citizen.CreateThread(function()
	exports["ucrp-keybinds"]:registerKeyMapping("", "Player", "Action Bar", "+actionBar", "-actionBar", "TAB")
	RegisterCommand('+actionBar', actionBarDown, false)
	RegisterCommand('-actionBar', actionBarUp, false)

	exports["ucrp-keybinds"]:registerKeyMapping("", "Player", "Wink", "+playerWink", "-playerWink")
...
AddEventHandler('ucrp-actionbar:hotreload', function()
...
Citizen.CreateThread( function()
	...
	while true do
      ...
      if weapon == "-37975472" then
        TriggerEvent("ucrp-weapons:threwSmokeGrenade")
        if lastEquippedItemToRemove then
          if exports["ucrp-inventory"]:hasEnoughOfItem(lastEquippedItemToRemove,1,false) then
        ...
        lastEquippedItemToRemove = nil
			...
			if throwableWeapons[weapon] and exports["ucrp-inventory"]:hasEnoughOfItem(weapon,1,false) then
				TriggerEvent("inventory:removeItem", weapon, 1)
				Citizen.Wait(3000)
			end
			if not excludedWeapons[hash] then
				nextLastShot = GetGameTimer()
				if nextLastShot - lastShot > 50000 then
					TriggerEvent("client:newStress", true, math.random(2,5), true)
				end
		...
		if IsPedPlantingBomb(ped) then
			if exports["ucrp-inventory"]:hasEnoughOfItem("741814745",1,false) then
...
AddEventHandler("ucrp-voice:focus:set", function(pState)
  focusTaken = pState
end)
...
AddEventHandler('equipWeaponID', function(hash,newInformation,sqlID,itemToRemove)
	--GiveAmmoNow()
	if not exports["ucrp-propattach"]:canPullWeaponHoldingEntity() then return end
  ...
  lastEquippedItemToRemove = itemToRemove
	...
	TriggerEvent("ucrp-weapons:client:weaponEquiped", hash, newInformation, sqlID, itemToRemove, armed)
	TriggerServerEvent("ucrp-weapons:weaponEquiped", hash, newInformation, sqlID, itemToRemove, armed)
...
function updateAmmoTable(newammo,ammoType)
	...
	TriggerServerEvent("ucrp-weapons:updateAmmo",newammo,ammoType,ammoTable)
end
...
RegisterNetEvent('ucrp-item:CheckClientAmmo')
AddEventHandler('ucrp-item:CheckClientAmmo', function(weapons)
	local ped = PlayerPedId()
...
function AttachmentCheck(weaponhash)
	...
	if exports["ucrp-inventory"]:hasEnoughOfItem("weapon_silencer_assault", 1, false, true) then
	...
	if exports["ucrp-inventory"]:hasEnoughOfItem("weapon_silencer_pistol", 1, false, true) then
	...
	if exports["ucrp-inventory"]:hasEnoughOfItem("weapon_oil_silencer", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 1532150734 )
	end

	if exports["ucrp-inventory"]:hasEnoughOfItem("weapon_scope", 1, false, true) then
	...
	if exports["ucrp-inventory"]:hasEnoughOfItem("weapon_uzi_extended", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0xE40F1CD2 )	
	end
	if exports["ucrp-inventory"]:hasEnoughOfItem("weapon_uzi_foldstock", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0x8FBBD54C )		
	end
	if exports["ucrp-inventory"]:hasEnoughOfItem("weapon_uzi_woodstock", 1, false, true) then
...
RegisterNetEvent("ucrp-weapons:hitPlayerWithCash", function(pTarget)
  TriggerServerEvent("ucrp-weapons:processGiveCashAmount", pTarget, lastEquippedInfo.amount, lastEquippedInfo.id)
end)
