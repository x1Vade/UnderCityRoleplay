...
--
local cmdsSet = {}
RegisterNUICallback("ucrp-ui:hudSetPreferences", function(data, cb)
    ...
    if data["hud.presets"] then
      ...
      for _, p in pairs(data["hud.presets"]) do
        ...
        if not cmdsSet[idx] then
          ...
          exports["ucrp-keybinds"]:registerKeyMapping("", "HUD", "Enable " .. s, "+hud_enable_" .. s, "-hud_enable_" .. s)
          RegisterCommand('+hud_enable_' .. s, function()
            exports["ucrp-ui"]:sendAppEvent("preferences", eventOptions)
          end, false)
    ...
    TriggerEvent("ucrp-preferences:setPreferences", data)
end)
...
AddEventHandler("ucrp-ui:watch", function()
...
AddEventHandler('ucrp-inventory:itemCheck', function (item, hasItem, quantity)
...
AddEventHandler("ucrp-admin:currentDebug", function(debugToggle)
...
AddEventHandler('ucrp-vehicles:pursuitMode', function (pEnabled, pMode, pModes)
...
AddEventHandler('ucrp-jail:attachedCollar', function (pAttached)
...
RegisterNetEvent('ucrp-boosting:client:setBoostCompletions')
AddEventHandler('ucrp-boosting:client:setBoostCompletions', function (pAmount)
...
RegisterNetEvent('ucrp-weapons:client:setWeaponFireRate')
AddEventHandler('ucrp-weapons:client:setWeaponFireRate', function (pAmount)
    weaponFireRate = pAmount
    setHudValue("weaponFireRate", weaponFireRate)
end)

RegisterNetEvent('ucrp-weapons:client:showWeaponFireRate')
AddEventHandler('ucrp-weapons:client:showWeaponFireRate', function (pToggle)
    showWeaponFireRate = pToggle
    setHudValue("showWeaponFireRate", pToggle)
end)

RegisterNetEvent('ucrp-ui:hud:forceShowMinimap', function(pForce)
...
function roundedRadar()
    ...
    Citizen.CreateThread(function()
        ...
        -- for k,v in pairs(exports["ucrp-base"]:getModule("Blips")) do
...
function getFuel()
    fuel = exports["ucrp-vehicles"]:CurrentFuel() or 0
end
    ...
    Citizen.CreateThread(function()
        while engineOn do
            ...
            if GetPedInVehicleSeat(veh, -1) == GetPed() then
                harnessDurability = exports["ucrp-vehicles"]:GetVehicleMetadata(veh, 'harness')
            end

            local degradation = exports["ucrp-vehicles"]:GetVehicleDegradation(veh)
...
AddEventHandler('ucrp-ui:setGaze', function (pEntity, pDelay)
    GazeEntity = pEntity
    GazeDelay = pDelay
end)
...
Citizen.CreateThread(function()
  while true do
    ...
    if toggleParachuteOff and pState == -1 then
      ...
      exports["ucrp-ui"]:sendAppEvent("hud", { hasParachute = false })
...
AddEventHandler("hud:equipParachute", function()
  hasParachute = true
  exports["ucrp-ui"]:sendAppEvent("hud", { hasParachute = true })
end)

AddEventHandler("ucrp-island:onIsland", function(pState)
    onIsland = pState
end)
