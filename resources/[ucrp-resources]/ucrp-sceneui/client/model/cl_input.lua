...
RegisterNUICallback('ucrp-ui:applicationClosed', function(data, cb)
    if (data.name ~= 'textbox' or data.callbackUrl ~= 'ucrp-ui:inputResponse') then return end
...
RegisterNUICallback('ucrp-sceneui:inputResponse', function(data, cb)
    ...
    exports['ucrp-sceneui']:closeApplication('textbox')
end)
...
function OpenInputMenu(pEntries, pValidation)
    ...
    exports['ucrp-sceneui']:openApplication('textbox', {
        callbackUrl = 'ucrp-ui:inputResponse',
        key = inputId,
        items = pEntries,
        show = true,
    })
