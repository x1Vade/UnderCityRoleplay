...
AddEventHandler('ucrp-config:configReady', function()
    loadPublicData()
end)

function loadPublicData()
    local showWatermark = exports['ucrp-config']:GetMiscConfig('hud.watermark')
    local showroomPurchase = exports['ucrp-config']:GetMiscConfig('showrooms.catalog.purchase')
    local skipMdwProfileCheck = exports['ucrp-config']:GetMiscConfig('mdw.profiles.skip')
    local serverCode = exports["ucrp-config"]:GetServerCode()
...
function sendCharacterData()
    Citizen.CreateThread(function()
        ...
        SendUIMessage({ source = "ucrp-nui", app = "character", data = character });
        
        local endpointData = exports["ucrp-fiber"]:GetGameDetails()
        SendUIMessage({ source = "ucrp-nui", app = "game", data = endpointData });

        Citizen.Wait(5000)

        TriggerEvent('ucrp-ui:phoneReady')
        TriggerServerEvent('ucrp-ui:phoneReady')
    end)
end

RegisterNetEvent("ucrp-spawn:characterSpawned")
AddEventHandler("ucrp-spawn:characterSpawned", function()
    charSpawned = true
    Citizen.CreateThread(function()
        DisplayRadar(0)
...
-- AddEventHandler("ucrp-ui:hud:values", function(values)
--   -- sendAppEvent("hud", {
--   --     food = values["hunger"],
