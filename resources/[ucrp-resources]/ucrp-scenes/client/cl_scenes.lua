...
RegisterNetEvent("ucrp-scenes:updateScenes", function(scenes)
    activeScenes = scenes
end)

RegisterNetEvent("ucrp-scenes:deleteScene")
AddEventHandler("ucrp-scenes:deleteScene", function(removeId)
    if removeId then
...
function calculateScenesToDraw()
    ...
    isPeekActive = exports['ucrp-interact']:IsPeeking()
...
RegisterCommand("+deleteScene", function()
    RPC.execute("ucrp-scenes:deleteScene", GetEntityCoords(PlayerPedId()))
end, false)
RegisterCommand("-deleteScene", function() end, false)

Citizen.CreateThread(function()
    exports["ucrp-keybinds"]:registerKeyMapping("", "Scenes", "Start / Place Scene", "+startScene", "-startScene")
    exports["ucrp-keybinds"]:registerKeyMapping("", "Scenes", "Enable / Disable", "+enableScene", "-enableScene")
    exports["ucrp-keybinds"]:registerKeyMapping("", "Scenes", "Delete Closest Scene", "+deleteScene", "-deleteScene")
    RequestStreamedTextureDict('commonmenu', true)
    Wait(5000)
    activeScenes = RPC.execute("ucrp-scenes:getScenes")
end)

AddEventHandler("ucrp-preferences:setPreferences", function(data)
...
RegisterNUICallback("newScene", function(data, cb)
    ...
    RPC.execute("ucrp-scenes:addScene", {
        pos = activePos,
        text = text,
        colour = colour,
        distance = distance,
