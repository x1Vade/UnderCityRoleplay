...
local function sellFish()
    ...
    function processFish(fish, bMarket)
        local qty = exports["ucrp-inventory"]:getQuantity(fish, true)
    ...
    if totalFish > 0 then
        --RPC.execute("ucrp-financials:giveJobPay", "fishing_sales", totalFish * pricePerFish)
    ...
    if totalBMarketFish > 0 then
        if nightTime then
            --local payoutFactor = exports["ucrp-config"]:GetModuleConfig("main").payoutFactor
...
AddEventHandler("ucrp-npcs:ped:sellFish", function()
    sellFish()
end)
