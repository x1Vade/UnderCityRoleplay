...
function check(){
    ...
    if(last_pos === 48){
        stopTimer();
        $.post('https://np-casino-generatorminigame/callback', JSON.stringify({'success': true}));
        trueMinigame();
        console.log(true)
        reset();
    }
...
function falseMinigame(){
    ...
    setTimeout(function(){
        finish();
        $.post('https://np-casino-generatorminigame/callback', JSON.stringify({'success': false}));
    },1500)
}
