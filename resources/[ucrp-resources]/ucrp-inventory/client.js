...
on('ucrp-base:playerSpawned', async (broughtData) => {
...
//  $.post("http://ucrp-inventory/SlotJustUsed", JSON.stringify({target: targetSlot, origin: originSlot, itemid: itemidsent }));
let recentused = [];
...
on('inventory:open_hidden', (penis, vehicleFound) => {
    let vehId = exports['ucrp-vehicles'].GetVehicleIdentifier(vehicleFound)
...
on('inventory-open-request', async () => {
  ...
  //  const apartmentFloor = exports["ucrp-apartments"].getModule("func").getApartment()
    let targetid = 0;
    cid = exports.isPed.isPed("cid");
    ...
    const currentTarget = exports['ucrp-target'].GetCurrentEntity()
    ...
    if (isInVehicle) {
        ...
        if (!IsThisModelABicycle(vehicleModel) && !IsThisModelABike(vehicleModel)) {
            ...
            const vehId = exports['ucrp-vehicles'].GetVehicleIdentifier(vehicleFound)
            if (!vehId) {
       ...
       // let serverCode = exports["ucrp-config"].GetServerCode();
...
on('ucrp-items:SetAmmo', (sentammoTable) => {
    if (sentammoTable) {
...
on('__cfx_nui:ServerCloseInventory', (data, cb) => {
    ...
    if (data.name != 'none') {
        ...
        emit('ucrp-inventory:closed', data.name);
    }
    cb({});
});
...
on('__cfx_nui:craftProgression', (data, cb) => {
    cb("ok");
    emit("ucrp-inventory:craftProgression", data);
    emitNet("ucrp-inventory:craftProgression", data);
});
...
function SetCustomNuiFocus(hasKeyboard, hasMouse) {
    ...
    //   if (HasNuiFocus === true) {
    //   	emit("ucrp-voice:focus:set", HasNuiFocus, hasKeyboard, hasMouse);
    //   } else {
    // 	  setTimeout(() => {if (HasNuiFocus !== true) emit("ucrp-voice:focus:set", false, false, false);}, 1000)
    //   }
}
...
exports('setPlayerWeight', (cid, weight) => {
    ...
    emitNet('ucrp-inventory:server:weightChange', cid, weight);
