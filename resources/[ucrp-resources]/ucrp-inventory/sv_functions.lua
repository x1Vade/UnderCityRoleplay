...
function getQuantity(pServerId, pItemId)
    local user, amount = exports["ucrp-base"]:getModule("Player"):GetUser(pServerId), 0
    ...
    if user then
        ...
        if checkQuality then
            decayDate = exports['ucrp-inventory']:GetDecayDate(pItemId)
...
function removeItem(pServerId, pItemId, pAmount, pShouldNotify)
    local user = exports["ucrp-base"]:getModule("Player"):GetUser(pServerId)
