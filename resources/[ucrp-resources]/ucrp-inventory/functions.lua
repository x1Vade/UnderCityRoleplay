...
local function processStressBlock()
    TriggerEvent("ucrp-buffs:applyBuff", "joints", { { buff = "stressblock", percent = 1.0 } })
end
...
AddEventHandler("ucrp-katana:removeEquippedKatana", function()
...
RegisterCommand('+useKeyFob', function()
    if Throttled("ucrp-doorss:doorKeyFob", 1000) then return end
    TriggerEvent("ucrp-doors:doorKeyFob")
end, false)
RegisterCommand('-useKeyFob', function() end, false)

Citizen.CreateThread(function()
    exports["ucrp-keybinds"]:registerKeyMapping("", "Vehicle", "Door Keyfob", "+useKeyFob", "-useKeyFob", "Y")
...
AddEventHandler('RunUseItem', function(itemid, slot, inventoryName, isWeapon, passedItemInfo)
    ...
    if parachuteConfig[itemid] and tonumber(ItemInfo.quality) > 0 and not IsPedInParachuteFreeFall(player) and not IsPedFalling(player) and (GetPedParachuteState(player) == -1 or GetPedParachuteState(player) == 0) then
        ...
        TriggerEvent("parachute:icon:enabled")
        justUsed = false
        retardCounter = 0
        lastCounter = 0
        return
        ...
        if GetEntityModel(PlayerPedId()) == `Mr_kebun` then
          if tonumber(ItemInfo.quality) > 0 then
              if not isKatanaEquipped then
                ...
                Citizen.CreateThread(function()
                    Citizen.Wait(1000)
                    TriggerServerEvent("ucrp-katana:cursedKatanaEquip", GetEntityCoords(PlayerPedId()))
                    TriggerEvent("ucrp-katana:cursedKatanaEquipC")
                end)
              else
                isKatanaEquipped = false
              end
    ...
    if stolenItems[itemid] and exports["ucrp-npcs"]:isCloseToPawnPed() then
    ...
    if (itemid == "pdbadge") then
        TriggerEvent("ucrp-badge:client:showbadge")
    end
    ...
    if (itemid == "heistlaptop3") then
        TriggerEvent('ucrp-fleeca:UseGreenLapTop')
    end 

    if (itemid == "heistlaptop2") then
        TriggerEvent('ucrp-paleto:UseBlueLapTop')
    end
    ...
    if (itemid == "lsdtab" or itemid == "badlsdtab") then
        ...
        local finished = exports["ucrp-taskbar"]:taskBar(3000,"Placing LSD Strip on 👅",false,false,playerVeh)
    ...
    if (itemid == "decryptersess" or itemid == "decrypterfv2" or itemid == "decrypterenzo") then
      if (#(GetEntityCoords(player) - vector3( 1275.49, -1710.39, 54.78)) < 3.0) then
          local finished = exports["ucrp-taskbar"]:taskBar(25000,"Decrypting Data",false,false,playerVeh)
          if (finished == 100) then
    ...
    if (itemid == "femaleseed") then
        ...
        TriggerEvent("ucrp-weed:plantSeed", itemid)
     end
    ...
    if (itemid == "weedoz") then
      ...
      local finished = exports["ucrp-taskbar"]:taskBar(5000,"Packing Q Bags",false,false,playerVeh)
        if (finished == 100) then
    ...
    if ( itemid == "smallbud" and hasEnoughOfItem("qualityscales",1,false) ) then
        local finished = exports["ucrp-taskbar"]:taskBar(1000,"Packing Joint",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "weedq") then
        local finished = exports["ucrp-taskbar"]:taskBar(1000,"Rolling Joints",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "qtakis") then
        ...
        TriggerEvent('ucrp-hud:ChangeHunger', 30)
    end
    ...
    if (itemid == "qdrpepper") then
        ...
        TriggerEvent('ucrp-hud:ChangeThirst', 30)
    end
    ...
    if (itemid == "lighter") then
        TriggerEvent("animation:PlayAnimation","lighter")
          local finished = exports["ucrp-taskbar"]:taskBar(2000,"Starting Fire",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "joint" or itemid == "joint2" or itemid == "customjointitem") then
        local finished = exports["ucrp-taskbar"]:taskBar(2000,"Smoking Joint",false,false,playerVeh)
        if (finished == 100) then
        ...
        if police >= 4 then
            ...
            if targetVehicle ~= 0 and GetHashKey("stockade") == GetEntityModel(targetVehicle) then
                ...
                if aDist < 2.0 then
                    ...
                    local finished = exports["ucrp-taskbar"]:taskBar(timeToOpen, "Unlocking Vehicle", false, false,
                        playerVeh)
                    if finished == 100 then
                        remove = true
    ...
    if (itemid == "heavyammo") then
        local finished = exports["ucrp-taskbar"]:taskBar(5000,"Reloading",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "pistolammo") then
        local finished = exports["ucrp-taskbar"]:taskBar(2500,"Reloading",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "flamethrowerammo") then
        local finished = exports["ucrp-taskbar"]:taskBar(20000,"Reloading flamethrower",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "huntingammo") then
        local finished = exports["ucrp-taskbar"]:taskBar(10000,"Reloading",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "rpgammo") then
        local finished = exports["ucrp-taskbar"]:taskBar(25000,"Reloading",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "taserammo") then
        local finished = exports["ucrp-taskbar"]:taskBar(2000,"Reloading",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "empammo") then
        local finished = exports["ucrp-taskbar"]:taskBar(30000,"Recharging EMP",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "armor" or itemid == "pdarmor") then
        ...
        local finished = exports["ucrp-taskbar"]:taskBar(4500,"Putting on Armor",true,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "cbrownie" or itemid == "cgummies") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = exports["ucrp-taskbar"]:taskBar(3000,"Consuming edibles 😉",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "bodybag") then
        local finished = exports["ucrp-taskbar"]:taskBar(10000,"Opening bag",true,false,playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "bodygarbagebag") then
            local finished = exports["ucrp-taskbar"]:taskBar(10000,"Opening trash bag",false,false,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "newaccountbox") then
        local finished = exports["ucrp-taskbar"]:taskBar(5000,"Opening present",false,false,playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "foodsupplycrate") then
        ...
        local finished = exports["ucrp-taskbar"]:taskBar(20000,"Opening the crate (ESC to Cancel)",false,false,playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "fishingtacklebox") then
        local finished = exports["ucrp-taskbar"]:taskBar(5000,"Opening",true,false,playerVeh)
        if (finished == 100) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
        end
    ...
    if (itemid == "housesafe") then
        if not hasEnoughOfItem("heavydutydrill", 1, false) then
            ...
            local finished = exports["ucrp-taskbar"]:taskBar(30000, "Cracking Safe...", true, false)
            if (finished == 100) then
                remove = true
                TriggerServerEvent('loot:useItem', itemid)
            end
    ...
    if (itemid == "fishingchest") then
        local finished = exports["ucrp-taskbar"]:taskBar(5000, "Opening Chest", true, false)
        if (finished == 100) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
        end
    ...
    if (itemid == "repcrate") then
        local finished = exports["ucrp-taskbar"]:taskBar(30000, "Opening...", true, false)
        if (finished == 100) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
        end
    ...
    if (itemid == "babyoil") then
        local finished = exports["ucrp-taskbar"]:taskBar(10000,"Applying baby oil to dome",true,false,playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "organcooler") then
        local finished = exports["ucrp-taskbar"]:taskBar(5000,"Opening cooler",true,false,playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "pixellaptop")  then
        TriggerEvent("ethicalpixel-boosting:client:usedlaptop")
    end    

    if (itemid == "trackerdisabler")  then
        TriggerEvent("ethicalpixel-boosting:useddisabler")
    end  

    if (itemid == "huntingbait")  then
        TriggerEvent("ucrp-hunting:usedBait")
    end


    if (itemid == "huntingknife")  then
        TriggerEvent("ucrp-hunting:skinAnimal")
    end

    if (itemid == "miningpickaxe")  then
        TriggerEvent("ucrp-start-mining")
    end
    ...
    if (itemid == "Bankbox") then
        if (hasEnoughOfItem("locksystem",1,false)) then
            local finished = exports["ucrp-taskbar"]:taskBar(10000,"Opening bank box.",false,false,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "Securebriefcase") then
        if (hasEnoughOfItem("Bankboxkey",1,false)) then
            local finished = exports["ucrp-taskbar"]:taskBar(5000,"Opening briefcase.",false,false,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "Largesupplycrate") then
        if (hasEnoughOfItem("2227010557",1,false)) then
            local finished = exports["ucrp-taskbar"]:taskBar(15000,"Opening supply crate.",false,false,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "xmasgiftcoal") then
        local finished = exports["ucrp-taskbar"]:taskBar(15000, "Opening Gift", false, false, playerVeh)
        if (finished == 100) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
        end
    ...
    if (itemid == "emptyballoon") then
        if (hasEnoughOfItem("nitrous",1,false)) then
            local finished = exports["ucrp-taskbar"]:taskBar(5000,"Filling Balloon.",false,false,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "Peecup_empty") then
        ...
        local finished = exports["ucrp-taskbar"]:taskBar(10000, "Peeing in cup.", false, false, playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "megaphone") then
      TriggerEvent("ucrp-usableprops:megaphone")
    end
        ...
        if IsPedInAnyVehicle(PlayerPedId(), true) then
            if not IsToggleModOn(currentVehicle,18) then
                ...
                Citizen.Wait(100)
                    ...
                    exports["ucrp-taskbar"]:closeGuiFail()
    ...
    if (itemid == "ciggy") then
        local finished = exports["ucrp-taskbar"]:taskBar(1000,"Lighting Up",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "oxygentank") then
        local finished = exports["ucrp-taskbar"]:taskBar(30000,"Oxygen Tank",true,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "softdrink") then
        ...
        TriggerEvent('ucrp-hud:ChangeThirst', 69)
    end
    ...
    if (itemid == "jailfood" or itemid == "bleederburger" or itemid == "heartstopper" or itemid == "torpedo" or itemid == "meatfree" or itemid == "moneyshot" or itemid == "fries") then
        ...
        TriggerEvent('ucrp-hud:ChangeHunger', 69)
    end
    ...
    or itemid == "experiment_09") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = exports["ucrp-taskbar"]:taskBar(3000,"Downing content of bottle. 💊",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "pillbox") then
        local finished = exports["ucrp-taskbar"]:taskBar(5000, "Opening pill box", false, false, playerVeh)
        if (finished == 100) then
    ...
    if itemid == "customfooditem" then
        TriggerEvent("ucrp-hud:ChangeHunger", 100)
    end
    ...
    if itemid == "kdragonwater" then
        ...
        AttachPropAndPlayAnimation(
          ...
          6000,
          "Drink",
          "ucrp-katana:transformKFromDrink",
          true,
          itemid,
          playerVeh
        )
    ...
    if (itemid == "methlabproduct") then
        local finished = exports["ucrp-ui"]:taskBarSkill(2400, math.random(12, 18))
        if finished == 100 then
            TriggerEvent("attachItemObjectnoanim","crackpipe01")
            TriggerEvent("Evidence:StateSet",2,1200)
            TriggerEvent("Evidence:StateSet",6,1200)
    ...
    if itemid == "ketamine" then
        local finished = exports["ucrp-ui"]:taskBarSkill(2000, math.random(10, 15))
        if finished == 100 then
            TriggerEvent("attachItemObjectnoanim","drugpackage01")
            TriggerEvent("Evidence:StateSet",2,1200)
            TriggerEvent("Evidence:StateSet",6,1200)
    ...
    if (itemid == "shitlockpick") then
        ...
        local finished = exports["ucrp-ui"]:taskBarSkill(1000, math.random(7, 12))
        if (finished == 100) then
    ...
    if (itemid == "harness") then
        ...
        if (PlayerPedId() == driver) then
            ...
            --TriggerServerEvent("vehicleMod:createHarness")
            remove = true

    TriggerEvent("ucrp-inventory:itemUsed", itemid, passedItemInfo, inventoryName, slot)
    TriggerServerEvent("ucrp-inventory:itemUsed", itemid, passedItemInfo, inventoryName, slot)
    ...
    if itemid == "roostertea" then
        AttachPropAndPlayAnimation("amb@world_human_drinking@coffee@male@idle_a", "idle_c", 49,15000,"Drink","ucrp-roostersrest:drinkTea",true,itemid,playerVeh)
    end
    ...
    if itemid == "franksflute" then
      TriggerServerEvent("ucrp-inventory:franksFlute", GetEntityCoords(PlayerPedId()))
    ...
    if itemid == "tattooremover" then
        local target = exports['ucrp-target']:GetCurrentEntity()
        --local target = PlayerPedId()
        if IsEntityAPed(target) and IsPedAPlayer(target) then
            TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_WELDING", 0, true)
            local finished = exports["ucrp-taskbar"]:taskBar(60000, "Removing tattoos")
    ...
    if itemid == "buddhaletter" then
        if GetEntityModel(PlayerPedId()) == `ig_buddha` or GetEntityModel(PlayerPedId()) == `g_m_m_buddha` then
            local finished = exports["ucrp-taskbar"]:taskBar(5000,"Opening letter.",false,false,playerVeh)
            if finished == 100 then
        ...
        if hasEnoughOfItem("buddhablade",1,false) and hasEnoughOfItem("buddhaguard",1,false) and hasEnoughOfItem("buddhahilt",1,false) then
            if hasEnoughOfItem("buddhamedalion",1,false) then
                ...
                if #(playerPos - vector3(-172.54, 319.13, 87.35)) < 10 then
                    local finished = exports["ucrp-taskbar"]:taskBar(30000,"Putting sword together.",false,false,playerVeh)
                    if (finished == 100) then
    ...
    if itemid == "buddhashovel" then
        ...
        if #(playerPos - vector3(-1911.78, 1388.1, 219.35)) < 10 then
            if BuddhaIsItemFound1 == false then
                ...
                local finished = exports["ucrp-taskbar"]:taskBar(15000, "Digging for something.", false, true, false, false, nil, 5.0, PlayerPedId())
                ClearPedTasks(PlayerPedId())
                if finished == 100 then
...
AddEventHandler("ucrp-inventory:attachPropPlayAnim", function(pType)
    ...
    TriggerEvent("ucrp-buffs:itemUsedSuccess", success)
end)
...
RegisterNetEvent('ucrp-business:repairItem')
AddEventHandler('ucrp-business:repairItem', function()
    
    local info = GetItemInfo(1)
    local Quality = info.quality
    local QualityCheck = (Quality < 80 and Quality or false)
    local PercentRepair = 80 - Quality
    local itemListInfo = json.decode(exports["ucrp-inventory"]:itemListInfo(info.item_id))
    local allowed = true  
...
RegisterNetEvent('ucrp-business:examineItem')
AddEventHandler('ucrp-business:examineItem', function()
...
function TaskItem(dictionary,animation,typeAnim,timer,message,func,remove,itemid,playerVeh,itemreturn,itemreturnid,quality)
    ...
    if timer > 0 then
        local finished = exports["ucrp-taskbar"]:taskBar(timer,message,true,false,playerVeh)
        if finished == 100 or timer == 0 then
            TriggerEvent(func, quality and quality or -1, itemid)
            if remove then
                if exports['ucrp-inventory']:hasEnoughOfItem(itemid, 1) then
                    TriggerEvent("inventory:removeItem", itemid, 1)
                else
                    print("duping bitch") 
                end
...
function checkForItems()
    ...
    for _, item in ipairs(items) do
        ...
        if hasItem and not PreviousItemCheck[item] then
            ...
            TriggerEvent('ucrp-inventory:itemCheck', item, true, quantity)
        elseif not hasItem and PreviousItemCheck[item] then
            PreviousItemCheck[item] = false
            TriggerEvent('ucrp-inventory:itemCheck', item, false, quantity)
        end
    end
end
...
AddEventHandler('inv:wellfed', function()
    ...
    TriggerEvent("changehunger")
    TriggerEvent("client:newStress", false, 100, false)
    TriggerEvent("changehunger")
...
AddEventHandler('inv:lockPick', function(isForced,inventoryName,slot)
    ...
    if targetVehicle == 0 then
        ...
        if driverPed ~= 0 then
            ...
            local finished = exports["ucrp-ui"]:taskBarSkill(15000,3)
            ...
            local finished = exports["ucrp-ui"]:taskBarSkill(2200,4)
        ...
        if targetVehicle ~= 0 and not isForced then
            ...
            local finished = exports["ucrp-ui"]:taskBarSkill(math.random(5000,10000),math.random(10,15))
            if finished ~= 100 then
                 lockpicking = false
                return
            end
            ...
            local finished = exports["ucrp-ui"]:taskBarSkill(1500,math.random(5,10))
            if finished ~= 100 then
...
AddEventHandler('veh:repairing', function(inventoryName,slot,itemid)
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.5 then
            ...
            local finished = exports["ucrp-ui"]:taskBarSkill(15000,math.random(10,20))
            if finished ~= 100 then
                fixingvehicle = false
                reapiring = false
            ...
            if finished == 100 then
                ...
                if myJob == "towtruck" then
                    ...
                    if advanced then
                        ...
                        local finished = exports["ucrp-ui"]:taskBarSkill(timer,math.random(4,7))
                        if finished ~= 100 then
                            fixingvehicle = false
                            reapiring = false
                            ClearPedTasks(playerped)
...
function getInformationFromFirstItem(itemToFind)
  local foundItem = exports["ucrp-inventory"]:GetInfoForFirstItemOfName(itemToFind)
...
AddEventHandler("ucrp-inventory:itemUsed", function(name, info)
...
RegisterUICallback("ucrp-ui:inv:moneycaseAction", function(data, cb)
	cb({ data = {}, meta = { ok = true, message = "done" } })
  exports['ucrp-ui']:closeApplication('textbox')
  RPC.execute("ucrp-inventory:doMoneycaseAction", data)
end)

AddEventHandler("ucrp-inventory:openGolfStore", function(name, info)
...
function openBurner(info)
    ...
    exports["ucrp-ui"]:openApplication("burner", info)
end
...
AddEventHandler("ammonium_bicarbonate:insert", function(pOriginInventory, pTargetInventory, pOriginSlot, pTargetSlot, pOriginItemId, pTargetItemId, pOriginItemInfo, pTargetItemInfo)
    ...
    local finished = exports["ucrp-taskbar"]:taskBar(5000 * quantity, "Breaking down into baggies...", false, false)
...
function notepadUsed()
    exports["ucrp-ui"]:openApplication("notepad", {
        canSave = true
    })
    TriggerEvent("animation:PlayAnimation", "notepad")
end

RegisterUICallback('ucrp-ui:cancelNotepadEmote', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = "done" } })
    TriggerEvent("animation:PlayAnimation", "cancel")
end)

RegisterUICallback('ucrp-ui:createNotepadNote', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = "done" } })

    exports["ucrp-ui"]:closeApplication("notepad")
