...
RegisterNetEvent("ucrp-hospitalization:checkin")
AddEventHandler("ucrp-hospitalization:checkin", function(allowed)
	if allowed then
		loadAnimDict('anim@narcotics@trash')
		TaskPlayAnim(PlayerPedId(),'anim@narcotics@trash', 'drop_front',1.0, 1.0, -1, 1, 0, 0, 0, 0)
		local finished = exports["ucrp-taskbar"]:taskBar(2500,"Checking Credentials")
...
RegisterNetEvent("ucrp-hospitalization:page")
AddEventHandler("ucrp-hospitalization:page", function()
	if exports["isPed"]:isPed("countems") >= 1 then 
		loadAnimDict('anim@narcotics@trash')
		TaskPlayAnim(PlayerPedId(),'anim@narcotics@trash', 'drop_front',1.0, 1.0, -1, 1, 0, 0, 0, 0)
		local finished = exports["ucrp-taskbar"]:taskBar(2500,"Paging a Doctor")
		if finished == 100 then
	...
	while true do 
		...
		if checkupCounter > 0 then
			...
			if hospitalization.level > 0 then
				if skipCheckup then
					TriggerEvent("client:newStress",true,math.random(20))
				end
			end
		end
	end
...
Citizen.CreateThread(function()
exports["ucrp-polytarget"]:AddCircleZone("pillbox_checkin", vector3(307.59, -595.3, 43.08), 0.35, {
