...
RegisterUICallback("ucrp-signin:handler", function(data, cb)
  local eventData = data.key
  if eventData.sign_in then
    TriggerServerEvent("ucrp-signin:duty", eventData.job)
...
RegisterUICallback("ucrp-signin:spawnVehicle", function(data, cb)
  local eventData = data.key
...
AddEventHandler("ucrp-signin:peekAction", function(pArgs, pEntity, pContext)
  ...
  exports["ucrp-ui"]:showContextMenu(MenuData[name])
end)
