RegisterNUICallback("data_status", function(data)
    if soundInfo[data.id] ~= nil then
        if data.type == "finished" then
            ...
            TriggerEvent("ucrp-musicplayer:songStopPlaying", data.id)
        end
...
RegisterNetEvent("ucrp-musicplayer:stateSound")
AddEventHandler("ucrp-musicplayer:stateSound", function(state, data)
