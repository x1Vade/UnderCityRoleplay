...
RegisterNetEvent('ucrp-base:playerSpawned')
AddEventHandler('ucrp-base:playerSpawned', function()
  Wait(1000)
...
RegisterNetEvent('ucrp-housing:rent_activate_cl')
AddEventHandler('ucrp-housing:rent_activate_cl', function()
...
AddEventHandler('ucrp-phone:sanitationIn', function()
  -- print("SANITATION SIGN IN")
  sanitation = true
end)

RegisterNetEvent("ucrp-phone:grabBackground")
AddEventHandler("ucrp-phone:grabBackground", function(link)
    wallPaper = link
end)
...
RegisterNUICallback('wallpaper', function(data, cb)
  ...
  TriggerEvent('ucrp-phone:grabBackground', wallPaperSelecionado)
...
AddEventHandler('phone:reset', function(cidsent)
    ...
    wallPaper = ""
    Wait(1)
    TriggerServerEvent('ucrp-phone:grabWallpaper')
end)
...
RegisterNUICallback('submitSettings', function(data, cb)
  ...
  Wait(5)
  if bg ~= "" then
    TriggerEvent('ucrp-phone:grabBackground', bg)
...
RegisterNUICallback('btnHousing', function()
  ...
  allNewHouses = {}
  myAccessHouse = {}
  local apt = RPC.execute("ucrp-phone:apt")
  local housing = exports['ucrp-housing']:housingForPhone()
  -- local apartment = exports['ucrp-apartments']:getMyCurrentApartment()
  myHousess = RPC.execute("ucrp-phone:getCurrentOwned")
  accessHouse = RPC.execute('ucrp-phone:getAccessHouse_2')
  ...
  for i,v in pairs(myHousess) do
    ...
    table.insert(newHouses, {
      ...
      cid = v.cid,
      status = v.status,
      cat = exports['ucrp-housing']:getHousingCatFromPropertID(v.hid),
  ...
  for l,s in pairs(accessHouse) do
    ...
    if tonumber(cid) ==  tonumber(s.cid) then
      table.insert(myAccessHouse, {
        ...
        status = s.status,
        -- name = s.name,
        cat = exports['ucrp-housing']:getHousingCatFromPropertID(s.house),
...
--   while true do
--     Wait(3000)
--     local myHouse = RPC.execute("ucrp-phone:getCurrentOwned")
...
RegisterNUICallback('sellVeh', function(data)
  ...
  TriggerServerEvent('ucrp-phone:sendVehBill', data.paypal, data.plate, data.amount)
end)

RegisterNetEvent('ucrp-phone:sellCar')
AddEventHandler('ucrp-phone:sellCar', function(plate,amount,sId)
  sellCarPlate = plate
...
RegisterNUICallback('purchaseCar', function(data)
  TriggerServerEvent('ucrp-phone:purchasedCar', sellCarPlate, data.amount, data.sID)
end)
...
function SpawnVehicle(vehicle, x,y,z, Fuel, customized, plate, IsViewing, enginehealth, bodyhealth)
  if DoesEntityExist(spawningVeh) then
	...
	Citizen.CreateThread(function()			
        ...
        exports['ucrp-vehicles']:SetVehicleIdentifier(spawningVeh, vid)
        ...
        if not IsViewing then    
            ...
            RPC.execute("ucrp-garages:states", "Out", plate, exports['ucrp-menu']:currentGarage(), pUpdatedFuel)
        end
    end)
end
...
RegisterNUICallback('sethGps', function(data)
 local house = exports["ucrp-housing"]:retrieveHousingTableMapped()
...
RegisterNUICallback('unlockHouses', function(data)
  ...
  if status == "Unlock" then
    TriggerEvent('ucrp-phone:h-Lock', hId,"Lock")
  else
    TriggerEvent('ucrp-phone:h-Lock', hId,"Unlock")
  end
  TriggerServerEvent('ucrp-phone:house:status',hId,status)
  Wait(200)
  TriggerEvent('updateHousing')
end)

RegisterNUICallback('editHousingOwn', function()
  -- if exports['ucrp-housing']:isNearProperty() then
    if exports["ucrp-housing"]:canEdit() then
      TriggerEvent('ucrp-housing:edit')
    else
      SendNUIMessage({
...
RegisterNUICallback('sellOwnHouse', function(data)
  local price = RPC.execute('ucrp-phone:getHousePrice', data.hid)
end)

RegisterNUICallback('giveAccess', function(data)
  local complete = RPC.execute('ucrp-phone:GiveAccess', data.hid,data.cid)
...
RegisterNUICallback('houseAccess', function(data)
  ...
  local access = RPC.execute('ucrp-phone:getAccessHouse', data.hid)
  local data = {
      {
        title = "Close",
        action = "ucrp-housing:close_access",
        key = {},
    },
  }
  local player = {}
  for i,k in pairs(access) do
    data[#data + 1] = {
          ...
          key = {"lockdown"},
          -- action = "ucrp-ui:apartmentsContext"
          children = {
            {
                title = "Remove Key",
  ...
  exports["ucrp-contexts"]:showContext(data)
end)

AddEventHandler("ucrp-housing:close_access", function()
  exports["ucrp-contexts"]:hideContext()
end)

AddEventHandler('ucrp-ui:apartmentsContext', function(data)
  RPC.execute("ucrp-phone:removeKeys",data[2],data[1])
  exports["ucrp-ui"]:hideContext(data)
end)
...
function hasDecrypt2()
    if exports["ucrp-inventory"]:hasEnoughOfItem("vpnxj",1,false,true) then
      return true
    else
      return false
    end
...
function hasTrucker()
    if exports["ucrp-base"]:getModule("LocalPlayer"):getVar("job") == "trucker"  then
      return true
    else
      return false
    end
...
function hasDecrypt()
    if exports["ucrp-inventory"]:hasEnoughOfItem("decrypterenzo",1,false) or exports["ucrp-inventory"]:hasEnoughOfItem("decryptersess",1,false) or exports["ucrp-inventory"]:hasEnoughOfItem("decrypterfv2",1,false) and not exports["isPed"]:isPed("disabled") or exports["ucrp-inventory"]:hasEnoughOfItem(80,1,false) and not exports["isPed"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function hasDevice()
    if exports["ucrp-inventory"]:hasEnoughOfItem("mk2usbdevice",1,false) and not exports["isPed"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function hasPhone()
    if
      (
      (exports["ucrp-inventory"]:hasEnoughOfItem("mobilephone",1,false) or 
      exports["ucrp-inventory"]:hasEnoughOfItem("stoleniphone",1,false) or 
      exports["ucrp-inventory"]:hasEnoughOfItem("stolens8",1,false) or
      exports["ucrp-inventory"]:hasEnoughOfItem("stolennokia",1,false) or
      exports["ucrp-inventory"]:hasEnoughOfItem("stolenpixel3",1,false) or
      exports["ucrp-inventory"]:hasEnoughOfItem("assphone",1,false) or
      exports["ucrp-inventory"]:hasEnoughOfItem("boomerphone",1,false))
...
function hasRadio()
    if (exports["ucrp-inventory"]:hasEnoughOfItem("radio",1,false) or exports["ucrp-inventory"]:hasEnoughOfItem("pdradio",1,false)) and not exports["isPed"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function openGui()
  ...
  if hasPhone() then
    ...
    local near = exports['ucrp-housing']:isNearProperty()
    local house = RPC.execute("ucrp-phone:getHouses")
...
function closeGui()
  ...
  -- TriggerEvent('ucrp-phone:UpdateStatePhone')
  pPhoneOpen = false
  recentopen = true
  Citizen.Wait(3000)
  recentopen = false
...
-- RegisterNetEvent('ucrp-phone:UpdateStatePhone')
-- AddEventHandler('ucrp-phone:UpdateStatePhone', function()
--     Wait(5)
...
RegisterNUICallback('updateMyWallpaper', function(data, cb)
  wallPaper = ""
  Wait(5)
  TriggerEvent('ucrp-phone:grabBackground', data.name .."?auto=compress&cs=tinysrgb&h=350")
...
AddEventHandler('phone:casino_paid', function(message)
  if exports['ucrp-phone']:pOpen() == false then 
    SendNUIMessage({openSection = "emailnotify", pEHandle = 'Payment Received.', pEMessages = message})
    SendNUIMessage({openSection = "phonemedio", timeout = "5200", pOpen = exports['ucrp-phone']:pOpen()}) 
  end
...
RegisterNetEvent('ucrp-spawn:confirm_spawned')
AddEventHandler('ucrp-spawn:confirm_spawned', function()
  serverSpawn = true
end)
...
RegisterNUICallback('jobcenter-setgps', function(data)
  ...
  if data.item == "oxy" then
    TriggerServerEvent("ucrp-deliveries:JobCenter")
...
RegisterNUICallback('checkHouse', function()
  ...
  if exports["ucrp-housing"]:canEdit() then
    TriggerEvent('ucrp-housing:phone:check')
  else
    SendNUIMessage({
      openSection = "noProperty"
    })
...
RegisterNUICallback('editHouse', function()
  print("EDITING")
  TriggerEvent('ucrp-housing:phone:check', "edit")
end)
...
AddEventHandler('ucrp-phone:sentHouse', function(hasPrice,id,street,price,category)
...
AddEventHandler('ucrp-phone:editingHouse', function(id,street,price)
  print("EDITING HERE")
  SendNUIMessage({
...
RegisterNUICallback('purchaseHouse', function(data)
  ...
  if rentEnabled then
     print("can purchase")
    --  TriggerEvent('ucrp-housing:buyed')
    --  action = "ucrp-housing:buyed",
    --         key = { 
...
-- AddEventHandler('ucrp-phone:noProperty', function()
...
RegisterNUICallback('editHousePrice', function(data)
  TriggerServerEvent('ucrp-housing:editPrice', data.hid,data.price)
end)

AddEventHandler('ucrp-phone:EnableSellRent', function()
    exports['ucrp-ui']:openApplication('textbox', {
    callbackUrl = 'ucrp-phone:SellRentEnable',
...
RegisterUICallback("ucrp-phone:SellRentEnable", function(data, cb)
  print('THIS IS ENABLED RENT/SALE FOR PLAYER', data[1].value,json.encode(data))
  TriggerServerEvent('ucrp-housing:rent_activate_sv', data[1].value)
  exports["ucrp-ui"]:hideContextMenu()
