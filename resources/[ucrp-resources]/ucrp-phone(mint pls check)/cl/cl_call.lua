...
AddEventHandler('phone:otherClientEndCall', function()
  ...
  Citizen.CreateThread(function()
    exports["ucrp-keybinds"]:registerKeyMapping("", "Player", "Hangup Call", "+hangup", "-hangup", "5")
    exports["ucrp-keybinds"]:registerKeyMapping("", "Player", "Answer Call", "+answer", "-answer", "6")
  end)
  ...
  AddEventHandler('phone:addToCall', function(voipchannel)
    exports['ucrp-voice']:addPlayerToCall(tonumber(voipchannel))
    -- if exports['ucrp-phone']:pOpen() == false then
    --   print("PHONE MEDIO 2") 
    --   -- SendNUIMessage({openSection = "phonemedio"}) 
    -- end
...
RegisterNetEvent('ucrp-phone:RemoveCall')
AddEventHandler('ucrp-phone:RemoveCall', function()
    SendNUIMessage({
      openSection = 'callnotifyEnd'
    })
    SendNUIMessage({
...
AddEventHandler('phone:removefromToko', function(playerRadioChannel)
  exports['ucrp-voice']:removePlayerFromCall()
end)
...
function endCall()
  ...
  mySourceID = 0
  TriggerEvent('phone:setCallState', isNotInCall)
  TriggerEvent('ucrp-phone:RemoveCall')
  onhold = false
  mySourceHoldStatus = false
  callid = 0
end
...
function checkForPayPhone()
  ...
  local ph = exports["ucrp-applications"]:KeyboardInput({
    header = "Payphone",
    rows = {
      {
        id = 1,
