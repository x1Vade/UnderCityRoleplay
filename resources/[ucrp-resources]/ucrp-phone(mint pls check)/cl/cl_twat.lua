...
RegisterNUICallback('btnTwatter', function()
    ...
    local twattermessages = RPC.execute("ucrp-phone:getTwatters")
    -- if not getTwat then
...
RegisterNUICallback('newTwatSubmit', function(data, cb)
    ...
    TriggerServerEvent('ucrp-phone:twatSendNotification', handle, data, data.time)
...
RegisterNUICallback("ucrp-phone:getTwatter", function(data, cb)
  cb({ data = currentTwats, meta = { ok = success, message = (not success and currentTwats or 'done') } })
end)



RegisterNetEvent('ucrp-phone:insertTwat')
AddEventHandler('ucrp-phone:insertTwat', function(name,msg,time,attach)
  table.insert(currentTwats, {
    handle = name,
    message = msg,
    attachment = attach,
