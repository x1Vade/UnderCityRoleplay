...
RegisterNUICallback('addEmployee', function(data)
    TriggerServerEvent('ucrp-phone:business_hired', data.state_id, data.rank, data.business)
end)

RegisterNUICallback('editEmployee', function(data)
  TriggerServerEvent('ucrp-phone:business_edit', data.rank, data.state_id, data.business)
end)
...
RegisterNUICallback('fireEmployee', function(data)
  ...
  if rank < 4 then
    ...
    TriggerServerEvent('ucrp-phone:business_fire', business, cid)
end)
...
RegisterNUICallback('payCheckEmployee', function(data)
    ...
    local complete, paid = RPC.execute('ucrp-phone:business_paycheck', business,cid,amount)
    if complete then
...
RegisterNUICallback('btnTaskGroups', function()
    ...
    -- local groups,bank,gIp = RPC.execute("ucrp-phone:Groups")
    SendNUIMessage({
      openSection = "groups",
      groups = groupObject
    })
  ...
  RegisterNUICallback('ucrp-phone:fireEmp', function(data,cb)
    TriggerServerEvent('ucrp-phone:fireEmp', data.id,data.group,data.name)
  end)

  RegisterNUICallback('addCashBank', function(data)
    local bid = data.bid
    local amount = data.amount
    local bank = RPC.execute("ucrp-phone:bus_addBank",bid,amount)
    if bank then
...
RegisterNUICallback('charge_customer', function(data)
  TriggerServerEvent('ucrp-phone:send_charge_customer', data.pPaypalInput, data.pChargeAmt)
end)

RegisterNetEvent('ucrp-phone:charge_customer_offer')
AddEventHandler('ucrp-phone:charge_customer_offer', function(pMoneyAmt)
    local title = "Business Charge"
    data = {amount = pMoneyAmt, sellerID = sId, pType = 'business_charge'}
    phoneBillNotification("", data, title)
end)

RegisterNUICallback('accept_bill', function(data)
  print(paypal)
end)