...
function getOrGeneratePhoneNumber (sourcePlayer, identifier, cb)
  ...
  local user = exports["ucrp-base"]:getModule("Player"):GetUser(sourcePlayer)
...
--     local src = source
--     local user = exports["ucrp-base"]:getModule("Player"):GetUser(tonumber(id))
--     local char = user:getCurrentCharacter()
  ...
  exports.ghmattimysql:execute("SELECT * FROM character_passes WHERE cid = @cid", {['cid'] = ply.id}, function(result)
      if result[1] ~= nil then
          ...
          if result[1].pass_type == "police" or result[1].pass_type == "ems" then
              ...
              TriggerClientEvent("ucrp-jobmanager:playerBecameJob", src, result[1].pass_type, result[1].pass_type, false)
          end
      end
  end)
end)
...
RegisterServerEvent("ucrp-phone:grabWallpaper")
AddEventHandler("ucrp-phone:grabWallpaper", function()
  ...
  local cid = char.id
    exports.ghmattimysql:execute("SELECT `wallpaper` FROM characters WHERE id = @id",{['id'] = cid}, function(data)
      if data[1].wallpaper ~= nil then
          TriggerClientEvent('ucrp-phone:grabBackground', src, data[1].wallpaper)
        --   print('Grabbbing',data[1].wallpaper)
      else
          TriggerClientEvent('ucrp-phone:grabBackground', src, "https://i.imgur.com/JVPavDg.jpg")
        --   print('Not Grabbbing')
      end
    end)
end)

RegisterServerEvent("ucrp-phone:GiveCrypto")
AddEventHandler("ucrp-phone:GiveCrypto", function(type,amount)
    local src = source
    local user = exports["ucrp-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("ucrp-phone:getChips", function(pSource)
    local src = source
    local user = exports["ucrp-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("ucrp-phone:getGroupSanitation", function(pSource)
    local src = source
    -- local user = exports["ucrp-base"]:getModule("Player"):GetUser(src)
    -- local char = user:getVar("character")
    -- local id = char.id
...
RPC.register("ucrp-phone:incomingCrypto", function(pSource)
    local src = source
    local user = exports["ucrp-base"]:getModule("Player"):GetUser(src)
...
RPC.register("ucrp-phone:blueprintid", function(pSource)
    return BPid
end)

-- RPC.register("ucrp-phone:buyCrypto", function(pSource,type,amount)
--     local src = source
--     local user = exports["ucrp-base"]:getModule("Player"):GetUser(src)
--     local char = user:getVar("character")
--     local id = char.id
...
RPC.register("ucrp-phone:exchangeCrypto", function(pSource,type,amount)
    local src = source
    local user = exports["ucrp-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("ucrp-phone:transferCrypto", function(pSource,type,amount,target)
    local src = source
    local user = exports["ucrp-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("ucrp-phone:GetSyndite", function (pSource)
    local src = source
    local user = exports["ucrp-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("ucrp-phone:GetTgb", function (pSource)
    local src = source
    local user = exports["ucrp-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("ucrp-phone:GetDvd", function (pSource)
    local src = source
    local user = exports["ucrp-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("ucrp-phone:buyCrypto", function (pSource,ctype,amount)
    local src = source
    local amount = amount
    local user = exports["ucrp-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
...
RPC.register("ucrp-phone:purchaseDarkMarket", function (pSource,type,price)
    local src = source
    local user = exports["ucrp-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local id = char.id
...
RPC.register('ucrp-phone:getMyGroup', function(cid)
...
RPC.register('ucrp-phone:getHouses', function(src)
...
RPC.register('ucrp-phone:apt', function(source)
    local src = source
    local user = exports["ucrp-base"]:getModule("Player"):GetUser(src)
...
RPC.register('ucrp-phone:getCurrentOwned', function(source)
    local src = source
    local user = exports["ucrp-base"]:getModule("Player"):GetUser(src)
    local cid = user:getCurrentCharacter().id
    local house = nil
...
RegisterNetEvent('ucrp-phone:house:status')
AddEventHandler('ucrp-phone:house:status', function(hid,status)
    -- local src = source
...
RPC.register('ucrp-phone:getHousePrice', function(source,hid)
    local src = source
    local user = exports["ucrp-base"]:getModule("Player"):GetUser(src)
...
RPC.register('ucrp-phone:GiveAccess', function(source,hid,cid)
    local src = source
    local hIds = hid.param
    local cid = cid.param
    local access
...
-- RPC.register('ucrp-phone:getAccessHouse', function(source)
--     local src = source
--     local user = exports["ucrp-base"]:getModule("Player"):GetUser(src)
--     local cid = user:getCurrentCharacter().id
--     local house
...
RPC.register('ucrp-phone:getAccessHouse', function(source,house)
    local src = source
    local user = exports["ucrp-base"]:getModule("Player"):GetUser(src)
    local cid = user:getCurrentCharacter().id
...
RPC.register('ucrp-phone:getAccessHouse_2', function(source,house)
    local src = source
    local user = exports["ucrp-base"]:getModule("Player"):GetUser(src)
    local cid = user:getCurrentCharacter().id
    local hid = house.param
...
RPC.register("ucrp-phone:removeKeys", function(src, pHouseId, pPlayerId)
    local src = source
    local user = exports["ucrp-base"]:getModule("Player"):GetUser(src)
...
-- RPC.register("ucrp-garages:states",function(pSource,state,plate,curGarage,fuel)
--     local src = source
--     local user = exports["ucrp-base"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
...
-- RPC.register("ucrp-phone:Groups", function(source,gIp)
--     local src = source
...
RegisterServerEvent('ucrp-phone:sendVehBill')
AddEventHandler('ucrp-phone:sendVehBill', function(paypal,plate,amount)
    src = source
    TriggerClientEvent('ucrp-phone:sellCar', paypal, plate, amount,src)
end)


RPC.register('ucrp-phone:sellVehicle', function(pSource,pl,py,am)
    local src = pSource
    ...
    local user = exports["ucrp-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local player = exports["ucrp-base"]:getModule("Player"):GetUser(paypal)
    local ply = user:getCurrentCharacter()
...
RegisterServerEvent('ucrp-phone:purchasedCar')
AddEventHandler('ucrp-phone:purchasedCar', function(plate,amount,sID)
    local src = source
    local user = exports["ucrp-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    ...
    if tonumber(char.bank) > tonumber(amount) then
        ...
        TriggerClientEvent('DoLongHudText', src, "You don't have enought money.",2)
    end
