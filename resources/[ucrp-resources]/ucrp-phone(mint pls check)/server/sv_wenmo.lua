RegisterNetEvent('ucrp-phone:transferWenmo', function(num,amount,comm)
    local src = source
    local zcid = getIdentifierByPhoneNumber(num)
    local user = exports["ucrp-base"]:getModule("Player"):GetUser(tonumber(src))
    ...
    if money[1] ~= nil then
        ...
        for _, player in ipairs(GetPlayers()) do
            local user = exports["ucrp-base"]:getModule("Player"):GetUser(tonumber(player))
            local char = user:getVar("character")
...
RPC.register('ucrp-phone:getWenmo', function(source)
    local src = source
    local user = exports["ucrp-base"]:getModule("Player"):GetUser(tonumber(src))
