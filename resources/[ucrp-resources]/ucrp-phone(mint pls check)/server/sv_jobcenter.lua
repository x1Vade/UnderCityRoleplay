...
RPC.register("ucrp-phone:getIdleGroup", function(source)
    local src = source
    local idle = {}
...
RPC.register("ucrp-phone:getMyGroup", function(source)
    local src = source
    local idle = {}
    local g = MySQL.query.await([[
        SELECT * FROM job_group
...
RPC.register("ucrp-phone:c_group", function(source)
    local src = source
...
RPC.register("ucrp-phone:j_group", function(source, id)
    local src = source
    local user = exports["ucrp-base"]:getModule("Player"):GetUser(src)
...
RPC.register("ucrp-phone:get_groupsMem", function(source, id)
...
RPC.register('ucrp-phone:g_ready', function(source,stat,id)
    local src = source
...
RPC.register('ucrp-phone:leave_group', function(source,id)
    local src = source
    local gId = id.param
...
--         end
--       end
--     --   TriggerLatentClientEvent('mhucrp-housing:collectHouses', -1, 30000, Houses)
--     end)
-- end)