...
$(document).ready(function () {
    ...
    $.post('http://ucrp-phone/getWeather', JSON.stringify({}));
    ...
    document.onkeyup = function (data) {
        ...
        if (data.which == 27) {
            ...
            $.post('http://ucrp-phone/close', JSON.stringify({}));
        }
    }
    ...
    $(".phone-screen").on('click', '#wifi', function (e) {
        if (wifizone){
            if (enablePassword) {
                ...
                $.post('https://ucrp-phone/connectWifi', JSON.stringify({}));
            }
        }
    });
    setInterval(function () {
        ...
        if (actionButton !== undefined) {
            switch (actionButton) {
                case "home":
                    ...
                    $.post('https://ucrp-phone/refreshMySMS', JSON.stringify({status: false}));
                    $.post('https://ucrp-phone/closeTwat', JSON.stringify({}));
                    $.post('https://ucrp-phone/checkNotify', JSON.stringify({}));
        ...
        if (action !== undefined) {
            switch (action) {
                case "yellow-pages-delete":
                    $.post('http://ucrp-phone/deleteYP', JSON.stringify({}));
                    break;
                case "racing-create":
                ...
                case "newPostSubmit":
                    e.preventDefault();
                    $.post('http://ucrp-phone/newPostSubmit', JSON.stringify({
                    ...
                    break;
                case "group-manage":
                    $.post('http://ucrp-phone/manageGroup', JSON.stringify({ GroupID: $(this).data('action-data') }));
                    break;
                case "btnTaskGang":
                    manageGroup = $(this).data('action-data');
                    $.post('http://ucrp-phone/btnTaskGang', JSON.stringify({}));
                    break;
                ...
                case "getCallHistory":
                    if (callStates[currentCallState] === "isCallInProgress" && currentContainer !== "incoming-call")
                        ...
                        $.post('http://ucrp-phone/' + action, JSON.stringify({}));
                    break;
                case "spotify":
    ...
    window.addEventListener('message', function (event) {
        ...
        if (item.openPhone === true) {
            ...
            $.post('http://ucrp-phone/checkModel', JSON.stringify({}));
        ...
        switch (item.openSection) {
            ...
            case "messageRead":
                ...
                $.post('https://ucrp-phone/refreshMySMS', JSON.stringify({status: true}));
            ...
            case "racing:hud:update":
                switch (item.hudState) {
                    ...
                    case "finished":
                        ...
                        drawRaceStats();
                        $.post('http://ucrp-phone/race:completed', JSON.stringify({
...
">${moment.utc(emails[email].time).local().fromNow()}
    ...
    $.post('http://ucrp-phone/emailReads', JSON.stringify({}));
}
...
function openBrowser(url) {
    ...
    $.post('http://ucrp-phone/btnCamera', JSON.stringify({}));
    $("#browser").fadeIn(300);
}
...
function purchaseDarkMarket(data) {
    $.post("https://ucrp-phone/purchase-darkMarket", JSON.stringify({item:data,itemamount}))
}

function trackvehicle(vehplate) {
    // console.log("VEH PLATE",vehplate)
    $.post("https://ucrp-phone/vehtrack", JSON.stringify({vehplate}))
}

function spawnVehicle(vehplate) {
    // console.log("VEH PLATE",vehplate)
    $.post("https://ucrp-phone/vehspawn", JSON.stringify({vehplate}))
}
...
$('.sellingvehicle').on('click', '#submit-input', function(){
    ...
    setTimeout(() => {
        $.post("https://ucrp-phone/sellVeh", JSON.stringify({
            plate: vplate,
            paypal: paypal,
            amount: amount
        }))
...
function gpsJobCenter(data) {    
    $.post("https://ucrp-phone/jobcenter-setgps", JSON.stringify({item:data}))
}
...
$("#selfieclose").click(function () {
    $('.selfieBox').hide();
    $.post('http://ucrp-phone/closeSelfi', JSON.stringify({}));
})

$("#settings").click(function () {

  $.post('http://ucrp-phone/openSettings', JSON.stringify({}));
})
...
function addYellowPage(item) {
    ...
    $('.yellow-pages-call').click(function(){
        ...
        $.post('http://ucrp-phone/callContact', JSON.stringify({
            name: '',
            number: callNumber
        }));
    })
...
$("#contact-submit-input").click(function (e) {
    ...
    setTimeout(() => {
        ...
        $.post('http://ucrp-phone/newContactSubmit', JSON.stringify({
            name: clean,
...
$(document).ready(function () {
    ...
    $('.send-ping').click(function () {
        ...
        // $.post('http://ucrp-phone/btnCamera', JSON.stringify({}));
        $.post('http://ucrp-phone/ucrp-ui:pingSend', JSON.stringify({
            number: targetId
        }));
        $('.send-value').val('');
    });
...
function updateSettings()
{
    switch (currentSettingWindow) {
        case "tokovoip":
            ...
            break;
        case "control":
            $.post('http://ucrp-phone/settingsUpdateToko', JSON.stringify({tag: "controlUpdate", controls: currentBinds}));
            break;
        case "browser":
            break;
        default:
...
function ResetSettings()
{
    switch (currentSettingWindow) {
        case "tokovoip":
            $.post('http://ucrp-phone/settingsResetToko', JSON.stringify());
            break;
        case "control":
            $.post('http://ucrp-phone/settingsResetControls', JSON.stringify());
            break;
        case "browser":
            break;
        default:
...
async function updateTokoSettings()
{
    ...
    await delayedLog();
    $.post('http://ucrp-phone/settingsUpdateToko', JSON.stringify({
        tag: "settings",
...
$('#racing-create-form').on('reset', function (e) {
    $.post('http://ucrp-phone/racing:map:cancel', JSON.stringify({}));
});

$('#racing-start-tracks').on('change', function (e) {
    let selectedMap = $(this).val();
    if(maps[selectedMap] !== undefined) {
        $.post('http://ucrp-phone/racing:map:removeBlips', JSON.stringify({}));
        $.post('http://ucrp-phone/racing:map:load', JSON.stringify({ id: selectedMap}));
...
$('#racing-start').submit(function (e) {
    ...
    $.post('http://ucrp-phone/racing:event:start', JSON.stringify({
...
$('#racing-create-form').submit(function (e) {
    e.preventDefault();
    $.post('http://ucrp-phone/racing:map:save', JSON.stringify({
...
$("#real-estate-sell-form").submit(function (e) {
    e.preventDefault();
    $.post('http://ucrp-phone/btnAttemptHouseSale', JSON.stringify({
...
$('#real-estate-transfer-form').submit(function (e) {
    e.preventDefault();
    $.post('http://ucrp-phone/btnTransferHouse', JSON.stringify({
...
$("#group-manage-pay-form").submit(function (e) {
    ...
    $.post('http://ucrp-phone/payGroup', JSON.stringify({
...
$("#group-manage-rank-form").submit(function (e) {
    e.preventDefault();
    $.post('http://ucrp-phone/promoteGroup', JSON.stringify({
        gangid: escapeHtml($(".group-manage-company-name").data('group-id')),
...
$("#group-manage-bank-form").submit(function (e) {
    ...
    $.post('http://ucrp-phone/bankGroup', JSON.stringify({
        gangid: escapeHtml($(".group-manage-company-name").data('group-id')),
...
$("#group-tasks-assign-modal-form").submit(function (e) {
    ...
    $.post('http://ucrp-phone/btnGiveTaskToPlayer', JSON.stringify({
...
$("#contacts-edit-submit").click(function (e) {
    ...
    $.post('http://ucrp-phone/editContactSubmit', JSON.stringify({
        name: clean,
    ...
    setTimeout(() => {
        $.post('http://ucrp-phone/refreshContacts', JSON.stringify({}));
    }, 2000);
...
$("#wallpaper-form").submit(function (e) {
    ...
    $.post('http://ucrp-phone/updateMyWallpaper', JSON.stringify({
        name: escapedName,
    }));
...
$("#stock-form").submit(function (event) {
    event.preventDefault();
    $.post('https://ucrp-phone/stocksTradeToPlayer', JSON.stringify({
...
$('#stock-transfer').click(function(){
    ...
    complateInputJustGreen()
    setTimeout(() => {
        $.post('https://ucrp-phone/exchangeCrypto', JSON.stringify({
...
$('#stock-submit').click(function(){
    ...
    complateInputJustGreen()
    setTimeout(() => {
        $.post('https://ucrp-phone/buyCrypto', JSON.stringify({
            stockID: escapeHtml(stockID),
            amount: escapeHtml(amount),
        }));
    }, 1000);
...
//     event.preventDefault();
//     $.post('http://ucrp-phone/newTwatSubmit', JSON.stringify({
...
$(".submit-bg").click(function (event){
    if ($('#bg-set-body').val() == '') {
        ...
        $.post('http://ucrp-phone/submitBg', JSON.stringify({
            bg: bg
        }));
...
$('#message-submit').click(function (event){
    ...
    $.post('http://ucrp-phone/newMessageSubmit', JSON.stringify({
    ...
    switch (currentContainer) {
        case "message":
            setTimeout(function () {
                ...
                $.post('http://ucrp-phone/messageRead', JSON.stringify({ sender: sender, receiver: receiver, displayName: displayName }));
            }, 300);
            break;
        case "messages":
            setTimeout(function () {
                $.post('http://ucrp-phone/messages', JSON.stringify({}));
            }, 300);
            break;
    }
})
...
$('#real-estate-evict-modal-accept').click(function () {
    $.post('http://ucrp-phone/btnEvictHouse', JSON.stringify({}));
...
$('.twatter-toggle-notification').click(function () {
    ...
    $.post('http://ucrp-phone/btnNotifyToggle', JSON.stringify({}));
});



$('.account-information-toggle-pager').click(function () {
    $.post('http://ucrp-phone/btnPagerToggle', JSON.stringify({}));
...
function clickImage(img) {
    $('.selfieBox').click(function(){
        copyToClipboard(img);
        $.post('http://ucrp-phone/closeSelfieBox', JSON.stringify({}));
    });
}
...
$('.racing-entries').on('click', '.racing-entries-join', function () {
    $.post('http://ucrp-phone/racing:event:join', JSON.stringify({ identifier: $(this).data('id') }));
});

$('.keys-entries').on('click', '.manage-keys', function () {
    $.post('http://ucrp-phone/retrieveHouseKeys', JSON.stringify({}));
});

$('.keys-entries').on('click', '.remove-shared-key', function(e) {
    $.post('http://ucrp-phone/removeSharedKey', JSON.stringify({
...
$('.manage-keys-entries').on('click', '.manage-keys-remove', function () {
    $.post('http://ucrp-phone/removeHouseKey', JSON.stringify({
...
$('.group-manage-entries').on('click', '.group-manage-fire', function () {
    ...
    $.post('http://ucrp-phone/ucrp-phone:fireEmp', JSON.stringify({ id: $(this).data('id'), group: $(this).data('group'), name: $(this).data('name') }));
});

$('.group-tasks-entries').on('click', '.group-tasks-track', function () {
    $.post('http://ucrp-phone/trackTaskLocation', JSON.stringify({ TaskIdentifier: $(this).data('id') }));
});

$('.delivery-job-entries').on('click', '.delivery-job-accept', function (e) {
    $.post('http://ucrp-phone/selectedJob', JSON.stringify({ jobType: $(this).data('job-type'), jobId: $(this).data('job-id') }));
});
...
$('.garage-entries').on('click', '.garage-spawn', function (e) {
    e.preventDefault();
    $.post('http://ucrp-phone/vehspawn', JSON.stringify({ vehplate: $(this).data('plate') }));
    $.post('http://ucrp-phone/btnGarage', JSON.stringify({}));
});

$('.garage-entries').on('click', '.garage-track', function () {
    $.post('http://ucrp-phone/vehtrack', JSON.stringify({ vehplate: $(this).data('plate') }));
});

$('.garage-entries').on('click', '.garage-pay', function (e) {
    $.post('http://ucrp-phone/vehiclePay', JSON.stringify({ vehiclePlate: $(this).data('plate') }));
    setTimeout(function () {
        $.post('http://ucrp-phone/btnGarage', JSON.stringify({}));
    }, 1500);
});
...
$('.gps-entries, .keys-entries').on('click', '.gps-location-click', function () {
    $.post('http://ucrp-phone/loadUserGPS', JSON.stringify({ house_id: $(this).data('house-id'), house_type: $(this).data('house-type') }));
})

$('.jss1481, .calls-container').on('click', '.btn-contacts-call', function () {
    $.post('http://ucrp-phone/callContact', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
...
$('#confirm-modal-accept').click(function (event) {
    $.post('http://ucrp-phone/removeContact', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
    $('#confirm-modal').modal('close');
});

$('#deleteting-yp').click(function (event) {
  $.post('http://ucrp-phone/deleteYP', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
});
...
$("#h-check").click(function () {
    ...
    $.post('http://ucrp-phone/checkHouse', JSON.stringify({}))
})

$("#edit-h").click(function () {
    // console.log("CHECK PROPERTY")
    // $.post('http://ucrp-phone/editHouse', JSON.stringify({}))
    noPropty()
})

$("#sell-h").click(function () {
    // console.log("CHECK PROPERTY")
    $.post('http://ucrp-phone/sellsHouse', JSON.stringify({}))
})
...
$('.h-found').on('click', '.h-found-purchase', function (event) {
    ...
    $.post('http://ucrp-phone/purchaseHouse', JSON.stringify({
...
$('.sell-house-own').on('click', '.confirm-sellHouse', function (event) {
    ...
    $.post('http://ucrp-phone/sellOwnHouse', JSON.stringify({
        hid: $('#h-p-id').val()
    }))
})
...
$('#submit-h-price').click(function(){
    ...
    $.post('http://ucrp-phone/editHousePrice', JSON.stringify({
        hid: hid,
        price: price
    }))
    $('.housing-edit-price').css('display', 'none');
...
function closePhoneShell() {
        ...
        $.post('https://ucrp-phone/refreshMySMS', JSON.stringify({status: false}));
        $.post('http://ucrp-phone/closeTwat')
        $('#twat-modal').modal('close');
        $('#contacts-container').css('display',"none")
    $('#contacts-container').css('display',"none")
    $("#calculator-container").css('display', 'none');
...
$('.phone-selfie').click(function(){
    $.post('http://ucrp-phone/phone:selfie', JSON.stringify({}));
    closePhoneShell()
})
...
$(document).on('click', '#twat-sendimg', function (e) {
    ...
    $.post('http://ucrp-phone/phone:selfie', JSON.stringify({}));
    e.preventDefault();
    $.post('http://ucrp-phone/PostNewImage', JSON.stringify({}),
        function (url) {
...
//                 $.post('http://ucrp-phone/btnAnswer', JSON.stringify({}));
//             }
//         });
...
//                             setTimeout(() => {
//                                 Notif.addClass('notification-container-fade-out')
//                                 $.post('http://ucrp-phone/btnHangup', JSON.stringify({}));
//                             }, 1000)
//                         }, 200)
//                     }
//                     // Notif.addClass('notification-container-fade-out')
...
//             }
//             Notif.addClass('notification-container-fade-out')
//             $.post('http://ucrp-phone/pingEnded', JSON.stringify({}));
//             setTimeout(() => {
//                 Notif.remove()
//                 var count = 0
...
//             $.post('http://ucrp-phone/ucrp-ui:pingAccept', JSON.stringify({}));
//             const Notif = $(".notification-container").filter(`[data-id="${Id}-ping"]`)
//                 if (Notif) {
//                     var count = 0
...
//             activePing = false
//             $.post('http://ucrp-phone/ucrp-ui:pingReject', JSON.stringify({}));
//             const Notif = $(".notification-container").filter(`[data-id="${Id}-ping"]`)
//                 if (Notif) {
//                     var count = 0
...
$(".house-entries, .access-house-entries").on('click', '.h-setgps', function (e) {
    ...
    $.post('http://ucrp-phone/sethGps', JSON.stringify({hid: overid}));
})
...
$(".house-entries, .access-house-entries").on('click', '.h-unlock', function (e) {
    ...
    $.post('http://ucrp-phone/unlockHouses', JSON.stringify({hid: overid, status: hstatus}));
 })
 $(".house-entries").on('click', '.h-edit-s', function (e) {
    $.post('http://ucrp-phone/editHousingOwn', JSON.stringify({}));
 })
 ...
 $(".house-entries").on('click', '.h-access-keys', function (e) {
    ...
    e.preventDefault();
    overid = $(this).attr("houseId")
    $.post('http://ucrp-phone/houseAccess', JSON.stringify({
        hid: overid
    }));
 })
 ...
 $('.submit-h-acess').click(function(){
    ...
    $('.phone-acces-house').hide()
    $.post('http://ucrp-phone/giveAccess', JSON.stringify({
        hid: hid,
        cid: cid
    }));
})
    ...
    $(document).ready(function () {
        $('.submit-settings').click(function(){
            ...
            $('.settings').hide()
                ...
                complateInputJustGreen();
                $.post('http://ucrp-phone/submitSettings', JSON.stringify({
                    bg: bg,
                    model: model,
                    twat: twat,
                    call: call
...
$("#cancel-charge-customer").click(function () {
    console.log("Ballz")
    $('.chargecustomer').css('display', 'none');
})

$("#send-charge-customer").click(function () {
    var pPaypalInput = escapeHtml($("#customer-form #pCharge-Input").val());
    var pChargeAmt = escapeHtml($("#customer-form #pCharge-Amount").val());

    $('.chargecustomer').css('display', 'none');
    complateInputJustGreen();

    $.post('http://ucrp-phone/charge_customer', JSON.stringify({
        pPaypalInput: pPaypalInput,
        pChargeAmt: pChargeAmt,
    }));
})