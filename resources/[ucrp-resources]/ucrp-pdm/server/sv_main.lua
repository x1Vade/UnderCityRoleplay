...
AddEventHandler('carshop:table', function(table)
    if table ~= nil then
        carTable = table
        TriggerClientEvent('ucrp-vehicleshop:returnTable', -1, carTable)
...
AddEventHandler('CheckMoneyForVeh', function(name, model,price,financed)
    ...
    if financed then
        ...
        local PaidBus = math.ceil(financedPrice * 0.50)
        if money >= financedPrice then
           ...
           -- exports["ucrp-banking"]:UpdateSociety(financedPrice, "car_shop", "add")
            TriggerEvent('society:update', PaidBus, "car_shop", "add")
        else
        ...
        if money >= price then
            ...
            TriggerClientEvent('FinishMoneyCheckForVeh', user.source, name, model, price, financed)
            TriggerClientEvent('menu:veh:purchase', user.source)
            --exports["ucrp-banking"]:UpdateSociety(price, "car_shop", "add")
            TriggerEvent('society:update', price, "car_shop", "add")
        else
            TriggerClientEvent('DoLongHudText', user.source, 'You dont have enough money on you!', 2)
            TriggerClientEvent('carshop:failedpurchase', user.source)
        end
...
AddEventHandler('BuyForVeh', function(platew, name, vehicle, price, financed)
    ...
    exports.oxmysql:execute("SELECT * FROM `characters_cars` WHERE license_plate = @license_plate", {['license_plate'] = platew}, function(result)
        if result[1] then
            ...
            TriggerClientEvent("ucrp-vehicleshop:update:plate", src, dbplate)
        else
            dbplate = platew
        end
    ...
    exports.oxmysql:execute("SELECT * FROM `characters_cars` WHERE license_plate = ?", {plateNumber}, function(data)
        if data[1] then
            ...
            if user:getBalance() >= CurrentPayment then
                ...
                TriggerEvent("ucrp:AddToMoneyLog", pSrc, "personal", -CurrentPayment, "withdraw", "N/A", (note ~= "" and note or "Car payment of $"..math.floor(CurrentPayment).." was taken out."))
            else
...
RegisterServerEvent("ucrp-vehicleshop:repo")
AddEventHandler("ucrp-vehicleshop:repo", function(plate)
    local src = source
    ...
    exports.oxmysql:execute("SELECT `license_plate`, `repoed` FROM `characters_cars` WHERE license_plate = ? AND finance_time = ? AND payments_left >= 1", {plate, "0"}, function(data)
        if data[1] then
            ...
            TriggerClientEvent("ucrp-vehicleshop:repo:success", src)
...
RegisterServerEvent("ucrp-vehicleshop:checkrepo")
AddEventHandler("ucrp-vehicleshop:checkrepo", function(plate)
    if plate == nil then
        return 
    end
            ...
            if data[1].repoed == tonumber(1) then
                if pFinace >= 2500 then
                    if user:getBalance() >= 5000 then
                        ...
                        TriggerClientEvent("ucrp-garages:takeout", src, data)
