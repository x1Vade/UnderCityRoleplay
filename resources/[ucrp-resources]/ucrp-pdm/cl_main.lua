...
local carspawns = {
	[1] =  { ['x'] = -41.19,['y'] = -1103.58,['z'] = 26.43,['h'] = 17.34, ['info'] = ' Car Spot 1' },
	...
	[5] =  { ['x'] = -48.36,['y'] = -1102.29,['z'] = 26.43,['h'] = 69.57, ['info'] = ' Car Spot 5' },
}
...
function updateCarTable(model,price,name)
	for i=1, #carTable do
		...
		TriggerServerEvent("ucrp-pdm:CarTablePDM",carTable)
	end
end

local myspawnedvehs = {}

RegisterNetEvent("ucrp-pdm:testdrive")
AddEventHandler("ucrp-:testdrive", function()
	if exports["isPed"]:GroupRank("police") >= 1 or #(vector3(-42.026374816895,-1094.5054931641,27.257934570312) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
RegisterNetEvent("ucrp-pdm:EnableBuy")
AddEventHandler("ucrp-pdm:EnableBuy", function()
	if #(vector3(-42.026374816895,-1094.5054931641,27.257934570312) - GetEntityCoords(PlayerPedId())) > 50.0 and exports["isPed"]:GroupRank("police") >= 1 then
		return
	end
	...
	TriggerServerEvent("ucrp-pdm:BuyEnabledSV",vehplate)
end)

RegisterNetEvent("ucrp-pdm:BuyEnabledCL")
AddEventHandler("ucrp-pdm:BuyEnabledCL", function(addplate)
	buyPlate[addplate] = true
	Citizen.Wait(60000)
	buyPlate[addplate] = nil
end)

RegisterNetEvent("ucrp-pdm:PDMCommission")
AddEventHandler("ucrp-pdm:PDMCommission", function(newAmount)
	if exports["isPed"]:GroupRank("police") >= 1 or #(vector3(-42.026374816895,-1094.5054931641,27.257934570312) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
RegisterNetEvent("ucrp-pdm:ReturnPDMTTable")
AddEventHandler("ucrp-pdm:ReturnPDMTTable", function(newTable)
	carTable = newTable
	DespawnSaleVehicles()
	SpawnSaleVehicles()
end)
...
function AttemptBuy(tableid)
	...
	TriggerServerEvent('ucrp-pdm:ChechMoney',name, model, price)
...
function DrawPrices()
	for i = 1, #carspawns do
		if #(vector3(carspawns[i]["x"],carspawns[i]["y"],carspawns[i]["z"]) - GetEntityCoords(PlayerPedId())) < 1.5 then
			...
			if exports["isPed"]:GroupRank("police") >= 1 then
				if buyPlate[addplate] ~= nil then
					exports['ucrp-interaction']:showInteraction("$" .. math.ceil(price) .. " | Com: %" ..commission.. " | [E] to change | [G] to buy.")
				else
					exports['ucrp-interaction']:showInteraction("$" .. math.ceil(price) .. " | Com: %" ..commission.. " | [E] to change")
				end
			else
				if buyPlate[addplate] ~= nil then
					exports['ucrp-interaction']:showInteraction("$" .. math.ceil(price) .. " [G] to buy.")
				else
					exports['ucrp-interaction']:showInteraction("Buy Price: $" .. math.ceil(price) .. " ")
				end
			end
...
function SpawnSaleVehicles()
	if not hasspawned then
		TriggerServerEvent("ucrp-pdm:RequestPDMTTable")
		Citizen.Wait(1500)
	end
...
function CloseCreator(name, veh, price, plate)
	Citizen.CreateThread(function()
		...
		if not boughtcar then
			...
			TriggerServerEvent('ucrp-pdm:BuyVehicle', plate, name, vehicle, price, VehicleProps)
			DespawnSaleVehicles()
			SpawnSaleVehicles()
			Citizen.Wait(100)
			exports['ucrp-interaction']:hideInteraction()
...
RegisterNetEvent("ucrp-pdm:FailedPurchase")
AddEventHandler("ucrp-pdm:FailedPurchase", function()
...
function ButtonSelected(button)
	...
	elseif this == "jobvehicles" or this == "tuners" or this == "casino" or this == "compacts" or this == "coupes" or this == "muscle" or this == "offroad" or this == "sedans" or this == "sports" or this == "sportsclassics" or this == "supers" or this == "suvs" or this == "vans" or this == "cycles" or this == "motorcycles" then
		if ownerMenu then
			...
			TriggerServerEvent('ucrp-pdm:ChechMoney',button.name, button.model, button.costs)
		end
	end
end
	...
	while true do
		...
		if vehshop.opened then
			...
			for i,button in pairs(menu.buttons) do
				...
				if exports["isPed"]:GroupRank("police") >= 1 and i >= vehshop.menu.from and i <= vehshop.menu.to then
...
RegisterCommand('commission', function(source, args, raw)
	if exports["isPed"]:GroupRank("police") >= 1 then
		...
		if tonumber(amount) > 0 and tonumber(amount) <= 60 then
			TriggerEvent('ucrp-pdm:PDMCommission', amount)
		else
...
RegisterCommand('testdrive', function(source, args, raw)
	if exports["isPed"]:GroupRank("police") >= 1 then
		TriggerEvent('ucrp-pdm:testdrive')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
	end
end)
...
RegisterCommand('enableBuy', function(source, args, raw)
	if exports["isPed"]:GroupRank("police") >= 1 then
		TriggerEvent('ucrp-pdm:EnableBuy')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
	end
end)