...
AddEventHandler('bank:deposit', function(amount, cid ,reason, statement)
  local src = source
  local user = exports["havok-base"]:getModule("Player"):GetUser(src)
  if (tonumber(user:getCash()) >= amount) then
    ...
    TriggerEvent('havok-base:bankdeposit', src, amount, tonumber(user:getCash()), tonumber(user:getBalance()))
  else
...
AddEventHandler('bank:withdraw', function(amount, cid , reason, statement)
  ...
  if (tonumber(user:getBalance()) >= amount) then
    ...
    TriggerClientEvent('isPed:UpdateCash', src, user:getCash())
    TriggerEvent('havok-base:bankwidthdraw', src, amount, tonumber(user:getCash()), tonumber(user:getBalance()))
  else
...
AddEventHandler('bank:givemecash', function(reciever, amount)
  local src = source
  local user = exports["havok-base"]:getModule("Player"):GetUser(src)
  local player = exports["havok-base"]:getModule("Player"):GetUser(tonumber(reciever))
  if tonumber(amount) <= user:getCash() then
    ...
    TriggerEvent('havok-base:bankgivecash', src, tonumber(receiver), pName, pSteam, pDiscord, amount, tonumber(user:getCash()), tonumber(user:getBalance()), amount, tonumber(player:getCash()), tonumber(player:getBalance()))
  else
...
AddEventHandler('bank:transfer', function(receiver, amount)
    local user = exports["havok-base"]:getModule("Player"):GetUser(source)
    local player = exports["havok-base"]:getModule("Player"):GetUser(tonumber(receiver))
    ...
    if tonumber(amount) <= user:getBalance() then
        ...
        TriggerEvent('havok-base:banktransfer', source, tonumber(receiver), pName, pSteam, pDiscord, amount, tonumber(user:getCash()), tonumber(user:getBalance()), amount, tonumber(player:getCash()), tonumber(player:getBalance()))
    else
...
AddEventHandler("bank:getLogs", function()
  local srcID = source
  local user = exports["havok-base"]:getModule("Player"):GetUser(srcID)
  local char = user:getCurrentCharacter()
...
RegisterServerEvent("havok-banking:GetCharacterBanking")
AddEventHandler("havok-banking:GetCharacterBanking", function(cid)
    local src = source
    exports.oxmysql:execute('SELECT * FROM character_passes WHERE `cid`= ?', {cid}, function(data)
    if data[1] ~= nil then 
      TriggerClientEvent('havok-banking:UpdatedAccounts', src, data)
    end
  end)
end)
...
RegisterServerEvent('havok-banking:addMoney')
AddEventHandler('havok-banking:addMoney', function(amount)
    local src = source
    local user = exports["havok-base"]:getModule("Player"):GetUser(src)
    user:addMoney(tonumber(amount))
end)

RegisterServerEvent('havok-banking:removeMoney')
AddEventHandler('havok-banking:removeMoney', function(amount)
    local src = source
    local user = exports["havok-base"]:getModule("Player"):GetUser(src)
    user:removeMoney(tonumber(amount))
end)