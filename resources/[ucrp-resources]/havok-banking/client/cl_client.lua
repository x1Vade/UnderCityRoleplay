...
RegisterNetEvent('havok-banking:UpdatedAccounts')
AddEventHandler('havok-banking:UpdatedAccounts', function(job)
...
function openGui(a)
  ...
  TriggerServerEvent('havok-banking:GetCharacterBanking', cid)
  local LocalPlayer = exports["havok-base"]:getModule("LocalPlayer")
...
RegisterNetEvent('havok-banking:ResetUI')
AddEventHandler('havok-banking:ResetUI', function()
  SetNuiFocus(false, false)
  SendNUIMessage({openBank = false})
  bankOpen = false
  atmOpen = false
    ...
    if IsNearATM() then
      if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
          ...
          if ( atmuse ) then 
              ...
              atmuse = false
              local finished = exports["havok-taskbar"]:taskBar(3000,"Retrieving Card")
              ClearPedTasks(PlayerPedId())
          else
              atmuse = true
        ...
        if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
            ...
            if ( atmuse ) then 
                ...
                atmuse = false
                local finished = exports["havok-taskbar"]:taskBar(1000,"Retrieving Card")
                ClearPedTasks(PlayerPedId())
            else
                atmuse = true
                TaskPlayAnim( player, "mp_common", "givetake1_a", 1.0, 1.0, -1, 49, 0, 0, 0, 0 )
...
RegisterNetEvent("havok-base:addedMoney")
AddEventHandler("havok-base:addedMoney", function(amt, total)
  TriggerEvent("banking:updateCash", total)
  TriggerEvent("banking:addCash", amt)
end)

RegisterNetEvent("havok-base:removedMoney")
AddEventHandler("havok-base:removedMoney", function(amt, total)
  TriggerEvent("banking:updateCash", total)
