...
local function benchObjectHandler(pBenchType, pItem)
    ...
    local id = exports['ucrp-objects']:PlaceAndSaveObject(
...
local itemHandlers = {
    ...
    ['pailshovel'] = function(info)
        ...
        local finished = exports["ucrp-taskbar"]:taskBar(15000, "Building Sandcastle", false, true, false, false, nil, 5.0, ped)
        ClearPedTasks(ped)
    ...
    ['letterbox'] = function(info)
        ...
        local properties = exports['ucrp-housing']:getHousingData()
        ...
        local name
        if elements then
            local prompt2 = exports['ucrp-ui']:OpenInputMenu(elements)
            if not prompt2 then
                return false
            end
            ...
            if mailType == 'property' then
                local housingTable = exports['ucrp-housing']:retriveHousingTable()
            ...
            elseif mailType == 'business' then
                local zones = exports['ucrp-polyzone']:GetZones(plyCoords)
                if not zones then
        ...
        local results = exports['ucrp-objects']:PlaceObject(
            model,
            {
                forceGroundSnap = true,
                zOffset = -0.05,
        ...
        local objId = RPC.execute('ucrp-usableprops:placeLetterbox', model, results[2].coords, results[2].rotation, mailType, foundInfo, name)
        if objId then
...
local function getMailboxInfo(pEntity)
    local object = exports['ucrp-objects']:GetObjectByEntity(pEntity)
    if not object then
        return false
    end
...
local function isPD()
    local job = exports["ucrp-base"]:getModule("LocalPlayer"):getVar("job")
...
-- Citizen.CreateThread(function()
--     exports["ucrp-interact"]:AddPeekEntryByModel({ `prop_sign_road_01a` }, {{
--         event = "ucrp-usableprops:stealStopSign",
--         id = "ucrp-usableprops:stealStopSign",
--         icon = "hammer",
--         label = "Steal!",
--         parameters = {},
--     }}, {
...
--     exports["ucrp-interact"]:AddPeekEntryByModel({ 2108567945, -1940238623 }, {{
--         event = "ucrp-usableprops:payParkingMeter",
--         id = "ucrp-usableprops:parking_pay",
--         icon = "dollar-sign",
...
--     exports["ucrp-interact"]:AddPeekEntryByModel({ `prop_skid_tent_01`, `prop_skid_tent_03` }, {{
--         event = "ucrp-usableprops:pickupTent",
--         id = "ucrp-usableprops:pickup_tent",
--         icon = "arrow-up",
--         label = "pickup tent",
--         parameters = {},
--     }}, {
--         distance = { radius = 2.0 },
--         isEnabled = function(pEntity)
--           return exports['ucrp-objects']:GetObjectByEntity(pEntity) ~= nil
--         end,
--     })

--     exports["ucrp-interact"]:AddPeekEntryByModel({ `prop_bench_04`, `prop_ld_bench01`, `prop_snow_bench_01` }, {{
--         event = "ucrp-usableprops:pickupBench",
--         id = "ucrp-usableprops:pickup_bench",
--         icon = "arrow-up",
--         label = "pickup bench",
--         parameters = {},
--     }}, {
...
--     exports["ucrp-interact"]:AddPeekEntryByModel({ `prop_letterbox_01`, `prop_letterbox_02`, `prop_letterbox_03`, `prop_letterbox_04`, `prop_postbox_01a` }, {{
--         event = "ucrp-usableprops:openMail",
--         id = "ucrp-usableprops:open_mailbox",
--         icon = "envelope",
--         label = "deposit mail",
--         parameters = {},
--     }}, {
...
--     exports["ucrp-interact"]:AddPeekEntryByModel({ `prop_letterbox_01`, `prop_letterbox_02`, `prop_letterbox_03`, `prop_postbox_01a` }, {{
--         event = "ucrp-usableprops:checkMail",
--         id = "ucrp-usableprops:check_mailbox",
--         icon = "envelope-open",
--         label = "check mail",
--         parameters = {},
--     }}, {
...
--             return (mailType == 'business' and (exports['ucrp-business']:HasPermission(info, 'stash_access') or isPD())) or
--             (mailType == 'property' and exports['ucrp-housing']:hasPropertyAccess(info, true)) or
--             (mailType == 'mrpd' and isPD()) or
--             (mailType == 'pillbox' and (job == 'ems' or job == 'doctor'))
--         end,
--     })

--     exports["ucrp-interact"]:AddPeekEntryByModel({ `prop_letterbox_01`, `prop_letterbox_02`, `prop_letterbox_03` }, {{
--         event = "ucrp-usableprops:removeMailbox",
--         id = "ucrp-usableprops:remove_mailbox",
--         icon = "trash-alt",
...
--         isEnabled = function(pEntity)
--             local info, mailType = getMailboxInfo(pEntity)
--             return mailType == 'business' and (exports['ucrp-business']:HasPermission(info, 'stash_access')) or
--             (mailType == 'property' and exports['ucrp-housing']:hasPropertyAccess(info, false))
--         end,
--     })

-- end)

AddEventHandler("ucrp-usableprops:stealStopSign", function(pArgs, pEntity, pContext)
    ...
    local finished = exports["ucrp-taskbar"]:taskBar(15000, "Stealing", false, true, false, false, nil, 5.0, ped)
    ClearPedTasks(ped)
    if finished == 100 then
...
RegisterNetEvent("ucrp-usableprops:deleteEntity", DeleteEntityAtCoords)
...
AddEventHandler("ucrp-usableprops:payParkingMeter", function(args, entity, context)
    PaidMeters[entity] = true
    local success = RPC.execute('ucrp-usableprops:payParkingMeter', args.price)
    if success then
        local pos = GetEntityCoords(entity)
        TriggerServerEvent("ucrp-scenes:addScene", {
...
AddEventHandler("ucrp-usableprops:pickupTent", function(args, entity, context)
    local tent = exports['ucrp-objects']:GetObjectByEntity(entity)
    if tent then
        local success = exports['ucrp-objects']:DeleteObject(tent.id)
        if success then
...
AddEventHandler("ucrp-usableprops:pickupBench", function(args, entity, context)
    local bench = exports['ucrp-objects']:GetObjectByEntity(entity)
    if bench then
        local success = exports['ucrp-objects']:DeleteObject(bench.id)
        if success then
...
local closeData = {}
AddEventHandler("ucrp-usableprops:openMail", function(args, entity, context)
    local info, mailType, object = getMailboxInfo(entity)
    waitForClose = RPC.execute('ucrp-usableprops:openMailbox', info, mailType, object.id)
...
AddEventHandler("ucrp-usableprops:checkMail", function(args, entity, context)
    if isPD() and mailType == 'business' then
        local prompt = exports['ucrp-ui']:OpenInputMenu({
    ...
    local info, mailType, object = getMailboxInfo(entity)
    RPC.execute('ucrp-usableprops:checkMailbox', info, mailType, object.id)
end)

AddEventHandler('ucrp-inventory:closed', function(pName)
    if waitForClose and pName == 'mailtemp:' .. closeData.mailType .. ':' .. closeData.objId then
        RPC.execute('ucrp-usableprops:closeMailbox', closeData.info, closeData.mailType, closeData.objId)
        waitForClose = false
        closeData = {}
    end
end)

AddEventHandler('ucrp-usableprops:removeMailbox', function(args, entity, context)
    TriggerEvent("animation:PlayAnimation","mechanic3")
    local finished = exports['ucrp-taskbar']:taskBar(15000, "Removing mailbox", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        local obj = exports['ucrp-objects']:GetObjectByEntity(entity)
        if not obj then
            return
        end
        exports["ucrp-objects"]:DeleteObject(obj.id)
    end
end)

AddEventHandler('ucrp-objects:objectCreated', function(pObject, handle)
    if pObject.data.metadata.mail then
        FreezeEntityPosition(handle, false)
    end
end)

AddEventHandler('ucrp-spawn:characterSpawned', function(pCharacterId)
    RPC.execute('ucrp-usableprops:checkMailAlerts', pCharacterId)
end)
