RPC.register("ucrp-weed:plantSeed", function(pSource, pCoords, pStrain)
    ...
    TriggerClientEvent("ucrp-weed:trigger_zone", -1, 2, {
...
RPC.register("ucrp-weed:addWater", function(pSource, pData)
    ...
    TriggerClientEvent("ucrp-weed:trigger_zone", -1, 3, returnData[1])

end)

RPC.register("ucrp-weed:addFertilizer", function(pSource, pData)
...
RPC.register("ucrp-weed:addMaleSeed", function(pSource, pData)
    ...
    TriggerClientEvent("ucrp-weed:trigger_zone", -1, 3, metaData[1])


    return true
end)

RPC.register("ucrp-weed:removePlant", function(pSource, pData)
    Await(SQL.execute("DELETE FROM _weed WHERE id = ?", pData))

    -- will get Fertilizer item
    TriggerClientEvent("ucrp-weed:trigger_zone", -1, 4, {
        id = pData
    })  
...
RPC.register("ucrp-weed:harvestPlant", function(pSource, pID)
    ...
    if ((os.time() - metaData[1].timestamp) / 60) >= PlantConfig.LifeTime then
        ...
        TriggerClientEvent("ucrp-weed:trigger_zone", -1, 4, metaData[1])
    else
        TriggerClientEvent("ucrp-weed:trigger_zone", -1, 4, metaData[1])
    end    
...
RPC.register("ucrp-weed:getPlants", function(pSource)
    ...
    TriggerClientEvent("ucrp-weed:trigger_zone", -1, 1, returnData)
    return true
end)