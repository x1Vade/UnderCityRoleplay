            ...
            if hit then
                ...
                for matHash,matType in pairs(MaterialHashes) do
                    if materialHash == matHash then
                        ...
                        local finished = exports["ucrp-taskbar"]:taskBar(3000, _L('weed-plants-plant-seed', 'Planting Seed'), false, true, false, false, nil, 5.0, PlayerPedId())
                        ClearPedTasks(PlayerPedId())
                        if finished == 100 then
                            RPC.execute("ucrp-weed:plantSeed", endCoords, typeMod)
...
RegisterNetEvent('ucrp-weed:checkPlant')
AddEventHandler("ucrp-weed:checkPlant", function(pContext, pEntity)
    local plantId = getPlantByPos(pContext["polyZoneCoordinates"])
    if not plantId then return end
    showPlantMenu(plantId)
end)

RegisterNetEvent('ucrp-weed:addWater')
AddEventHandler('ucrp-weed:addWater', function (pPlantId)
    playPourAnimation()
    local finished = exports["ucrp-taskbar"]:taskBar(2000, "Adding Water", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        local success = RPC.execute("ucrp-weed:addWater", pPlantId)
        if not success then
...
RegisterNetEvent('ucrp-weed:addFertilizer')
AddEventHandler('ucrp-weed:addFertilizer', function (pPlantId, key)
    playPourAnimation()
    local finished = exports["ucrp-taskbar"]:taskBar(2000, _L('weed-plants-add-fertilizer', 'Adding Fertilizer'), false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        local success = RPC.execute("ucrp-weed:addFertilizer", {["id"] = pPlantId, ["type"] = key})
        if not success then
...
RegisterNetEvent('ucrp-weed:addMaleSeed')
AddEventHandler('ucrp-weed:addMaleSeed', function (pPlantId)
    TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_GARDENER_PLANT", 0, true)
    local finished = exports["ucrp-taskbar"]:taskBar(3000, "Adding Male Seed", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        RPC.execute("ucrp-weed:addMaleSeed", pPlantId)
        TriggerEvent("inventory:removeItem", "maleseed", 1)
    end
    showPlantMenu(pPlantId)
end)

RegisterNetEvent('ucrp-weed:removePlant')
AddEventHandler('ucrp-weed:removePlant', function ( pPlantId)
    TriggerEvent("animation:PlayAnimation","layspike")
    local finished = exports["ucrp-taskbar"]:taskBar(3000, _L('weed-plants-remove', 'Removing'), false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        local getFertilizer = getPlantGrowthPercent(getPlantById( pPlantId)) > 20.0
        local success = RPC.execute("ucrp-weed:removePlant", pPlantId, getFertilizer)
        if not success then
...
RegisterNetEvent('ucrp-weed:pickPlant')
AddEventHandler("ucrp-weed:pickPlant", function(pContext, pEntity)
    ...
    local plyWeight = exports["ucrp-inventory"]:getCurrentWeight()
    ...
    TriggerEvent("animation:PlayAnimation","layspike")
    local finished = exports["ucrp-taskbar"]:taskBar(10000, _L('weed-plants-harvest', 'Harvesting'), false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        RPC.execute("ucrp-weed:harvestPlant", plantId)
    end
end)

AddEventHandler("ucrp-polyzone:enter", function(zone, data)
    if zone == "ucrp-weed:weed_zone" then
        inZone = inZone + 1
        if inZone == 1 then
            RPC.execute("ucrp-weed:getPlants")
        end
    end
end)

AddEventHandler("ucrp-polyzone:exit", function(zone, data)
    if zone == "ucrp-weed:weed_zone" then
        inZone = inZone - 1
...
RegisterNetEvent('ucrp-weed:trigger_zone')
AddEventHandler("ucrp-weed:trigger_zone", function (type, pData)
    --Update all plants
    if type == 1 then
...
function showPlantMenu(pPlantId)
    ...
    if growth < PlantConfig.HarvestPercent then
        context[#context+1] = {
            ...
            args = { pPlantId },
            event = 'ucrp-weed:addWater',
            disabled = not exports["ucrp-inventory"]:hasEnoughOfItem("water", 1, false)
        }

        context[#context+1] = {
            title = "Add Fertilizer",
            event = 'ucrp-weed:addFertilizerMenu',
            args = { pPlantId },
            disabled = not exports["ucrp-inventory"]:hasEnoughOfItem("fertilizer", 1, false)
        }
    end
    ...
    if getStageFromPercent(growth) < 3 and plant.gender == 0 then
        context[#context+1] = {
            title = "Add Male Seed",
            args = { pPlantId },
            event = "ucrp-weed:addMaleSeed",
            description = "Make the plant preggies",
            disabled = not exports["ucrp-inventory"]:hasEnoughOfItem("maleseed", 1, false)
        }
    end
...
RegisterNetEvent('ucrp-weed:addFertilizerMenu')
AddEventHandler("ucrp-weed:addFertilizerMenu", function (pPlantId)
    local context = {}

    context[#context+1] = {
        title = "Add Fertilizer (N)",
        args = { pPlantId, "n" },
        event = 'ucrp-weed:addFertilizer',
    }
