    ...
    data = {
        {
            ...
            icon = "handshake",
            event = "ucrp-weed:startCornering",
            parameters = {}
        }
    },
    options = {
        ...
        isEnabled = function(pEntity, pContext)
            local hasWeed = exports["ucrp-inventory"]:hasEnoughOfItem("weedbaggie", 1, false, true)
        --    local isCorneringAlready = exports["ucrp-meth"]:IsCornering()
    ...
    data = {
        {
            ...
            event = 'ucrp-weed:prepareBaggies',
            parameters = {},
        }
    },
    options = {
        distance = { radius = 5.0 },
        isEnabled = function(pEntity, pContext)
            local hasWeed = exports["ucrp-inventory"]:hasEnoughOfItem("weedpackage", 1, false, true)
            return hasWeed
        end
    }
}
...
Citizen.CreateThread(function()
    exports["ucrp-interact"]:AddPeekEntryByEntityType(InteractCorner.group, InteractCorner.data, InteractCorner.options)
    exports["ucrp-interact"]:AddPeekEntryByEntityType(InteractCornerVehicle.group, InteractCornerVehicle.data, InteractCornerVehicle.options)
    exports["ucrp-interact"]:AddPeekEntryByEntityType(InteractStartCorner.group, InteractStartCorner.data, InteractStartCorner.options)
    exports["ucrp-interact"]:AddPeekEntryByEntityType(InteractStopCorner.group, InteractStopCorner.data, InteractStopCorner.options)
end)

AddEventHandler("ucrp-weed:startCornering", function(pContext, pEntity)
    ...
    local canCorner, message, engMessage = RPC.execute("ucrp-weed:startCorner", CurrentCornerCoords)
    ...
    Citizen.CreateThread(function()
        ...
        while CurrentlyCornering do
            ...
            if not foundPed then
                ...
                RPC.execute("ucrp-weed:cornerPed", coords, NetworkGetNetworkIdFromEntity(foundPed), NetworkGetNetworkIdFromEntity(CurrentCornerVehicle))
            end
...
AddEventHandler("ucrp-weed:cornerSale", function(pContext, pEntity)
    if not DoesEntityExist(pEntity) == 1 or IsPedDeadOrDying(pEntity) then return end
    local baggies = exports["ucrp-inventory"]:getItemsOfType("weedbaggie", 1, true)
    ...
    RPC.execute("ucrp-weed:cornerSyncHandoff", CurrentCornerCoords, NetworkGetNetworkIdFromEntity(pEntity))
    ...
    local didSale = RPC.execute("ucrp-weed:cornerSale", CurrentCornerCoords, NetworkGetNetworkIdFromEntity(pEntity), CurrentCornerZone, baggieInfo)
    if didSale then
...
AddEventHandler("ucrp-weed:prepareBaggies", function(pContext, pEntity)
    if CornerConfig.RequireScales then
        local hasScale = exports["ucrp-inventory"]:hasEnoughOfItem("qualityscales", 1, false) or exports["ucrp-inventory"]:hasEnoughOfItem("smallscales", 1, false)
    ...
    local hasBaggies = exports["ucrp-inventory"]:hasEnoughOfItem("emptybaggies", CornerConfig.BaggiesPerBrick / 4, false)
    ...
    local finished = exports["ucrp-taskbar"]:taskBar(30000, _L('weed-corner-prepare', 'Preparing'), false, true, false, false, nil, 5.0, pEntity)
    if finished == 100 then
        local weedpackages = exports["ucrp-inventory"]:getItemsOfType("weedpackage", 1, true)
        if #weedpackages == 0 then return end
        local package = weedpackages[1]
        local info = json.decode(package.information)
        RPC.execute("ucrp-weed:prepareBaggies", info)
    end
end)

AddEventHandler("ucrp-weed:stopCornering", function(pContext, pEntity)
    TriggerEvent("DoLongHudText", _L('weed-corner-not-selling', 'No longer selling...'))
    stopCornering()
end)

RegisterNetEvent("ucrp-weed:addCorneredPed")
AddEventHandler("ucrp-weed:addCorneredPed", function(pPed)
    local ped = NetworkGetEntityFromNetworkId(pPed)
    ActivePeds[ped] = false
end)

RegisterNetEvent("ucrp-weed:cornerSyncHandoff")
AddEventHandler("ucrp-weed:cornerSyncHandoff", function(pPed)
    local ped = NetworkGetEntityFromNetworkId(pPed)
...
RegisterNetEvent("ucrp-weed:cornerPed")
AddEventHandler("ucrp-weed:cornerPed", function(pPed, pCornerCoords, pVehicle)
    local ped = NetworkGetEntityFromNetworkId(pPed)
    ActivePeds[ped] = true
    UsedPeds[ped] = true
    if NetworkHasControlOfEntity(ped) then
...
RegisterNetEvent("ucrp-weed:cleanCornerPeds")
AddEventHandler("ucrp-weed:cleanCornerPeds", function()
    UsedPeds = {}
end)
...
function stopCornering()
    ...
    RPC.execute("ucrp-weed:stopCorner")
end
