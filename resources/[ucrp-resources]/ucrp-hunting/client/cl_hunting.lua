...
Citizen.CreateThread(function()
    local result = RPC.execute("ucrp-hunting:getSettings")
...
AddEventHandler("ucrp-inventory:itemUsed", function(item)
    if item == "huntingbait" then
        ...
        local finished = exports["ucrp-taskbar"]:taskBar(15000, "Placing Bait", false, true, false, false, nil, 5.0, PlayerPedId())
    ...
    if item == "huntingknife" then
        ...
        TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_GARDENER_PLANT", 0, true)
        local finished = exports["ucrp-taskbar"]:taskBar(30000, "Preparing Animal", false, true, false, false, nil, 5.0, targetedEntity)
        if finished ~= 100 then
            ClearPedTasksImmediately(PlayerPedId())
        ...
        TriggerServerEvent('ucrp-hunting:skinReward',  NetworkGetNetworkIdFromEntity(myAnimal), mySpawn, illegalSpawn, animalName)
        DeleteEntity(myAnimal)
     --   RPC.execute("ucrp-hunting:getSkinnedItem", NetworkGetNetworkIdFromEntity(myAnimal), mySpawn, illegalSpawn, animalName)
        TriggerEvent("client:newStress", true, 8)
    end
end)
...
AddEventHandler("ucrp-inventory:itemUsed", function(item)
    if item == "huntingknife" then
        ...
        ClearPedTasksImmediately(PlayerPedId())
        TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_GARDENER_PLANT", 0, true)
        local finished = exports["ucrp-taskbar"]:taskBar(30000, "Preparing", false, true, false, false, nil, 5.0, targetedEntity)
        if finished ~= 100 then
            ClearPedTasksImmediately(PlayerPedId())
            TriggerEvent("DoLongHudText", "Preparing Cancelled", 2)
            return
