...
function GetEntityContext(pEntity, pEntityType, pEntityModel)
    ...
    if context.type == 1 then
        context.flags = exports["ucrp-flags"]:GetPedFlags(pEntity)
    ...
    elseif context.type == 2 then
        context.flags = exports["ucrp-flags"]:GetVehicleFlags(pEntity)
    elseif context.type == 3 then
        context.flags = exports["ucrp-flags"]:GetObjectFlags(pEntity)
    end
...
function GetPedContext(pEntity, pContext)
    ...
    if pContext.flags["isJobEmployer"] then
        pContext.job = exports["ucrp-jobs"]:GetNPCJobData(npcId)
    end
end

--[[function isDisabled()
    return exports["ucrp-base"]:getVar("dead") or
        exports["ucrp-base"]:getVar("handcuffed")
end]]

function hasPhone()
    return exports["ucrp-inventory"]:hasEnoughOfItem("mobilephone", 1, false) or
...
function hasKeys(pEntity)
    ...
    local hasKeys = exports["ucrp-vehicles"]:HasVehicleKey(pEntity)
...
function hasRepairItems(type)
    ...
    for i, v in pairs(items) do
        if exports["ucrp-inventory"]:hasEnoughOfItem(v, 1, false) then
            return true
        end
    end
...
AddEventHandler("ucrp-polyzone:enter", function(zone, data)
    if zone == "ucrp-jobs:impound:dropOff" then IsImpoundDropOff = true end
...
RegisterNetEvent("ucrp-jobs:jobChanged")
AddEventHandler("ucrp-jobs:jobChanged", function(pJobId)
    CurrentJob = pJobId
