...
function hasJobPermission(pJob, pPermission)
  ...
  local hasJobPermission = function()
    if pPermission == "employee" then
      ...
      return RPC.execute('ucrp-business:hasPermission', pJob, pPermission)
    end
  end
...
RegisterNetEvent("ucrp-interact:bustVehicleOwnerShipCache", function(pEntity)
...
function getVehicleOwner(pEntity)
  ...
  local vin = exports['ucrp-vehicles']:GetVehicleIdentifier(pEntity)
  local vehicleOwner = RPC.execute("ucrp-ottosauto:getVehicleOwner", vin)
...
AddEventHandler("ucrp-fishing:currentlyRentedBoat", function(handle)
