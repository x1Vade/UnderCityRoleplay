...
function StartPeekin()
    ...
    if LocalPlayer.state.disabled or LocalPlayer.state.disableinput or IsPedInAnyVehicle(PlayerPedId()) or IsPlayerFreeAiming(PlayerId()) or LocalPlayer.state.dead or IsPedBeingStunned(PlayerPedId()) or IsPedRagdoll(PlayerPedId()) or IsPedArmed(PlayerPedId(), 6) then return end
    ...
    UpdateRequired = true
    NearZones = {}
    nearbyZones = exports["ucrp-polytarget"]:GetZones(GetEntityCoords(PlayerPedId())) or {}
...
RegisterNetEvent("ucrp-target:changed", function(pTarget, pEntityType, pEntityOffset)
...
RegisterNetEvent('ucrp-polyzone:enter', function(zoneName, zoneData, zoneCenter)
...
RegisterNetEvent('ucrp-polyzone:exit', function(zoneName)
    if not PeekEntries['polytarget'][zoneName] then return end
...
Citizen.CreateThread(function()
    ...
    exports["ucrp-binds"]:registerKeyMapping("", "Player", "Peek at Target", "+peek", "-peek", "LMENU") 
end)
