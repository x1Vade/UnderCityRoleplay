...
AddEventHandler('ucrp-polyzone:entered:garages', polyZoneEnter)
AddEventHandler('ucrp-polyzone:exited:garages', polyZoneExit)
...
AddEventHandler('ucrp-polyzone:enter', function(name)
    ...
    for k, v in pairs (approvedGarages) do
        if name == v then
            exports["ucrp-ui"]:showInteraction("Garage")
            isAtGarage = true
            pCurrentGarage = name
        end
    end
    for k, v in pairs (sharedGarages) do
        if name == v then
            local garageData = exports["ucrp-garages"]:getGarageData(name)
