...
RegisterNUICallback("ucrp-ui:tunerapplication-closed", function(data, cb)
...
RegisterNUICallback("ucrp-ui:tunershowroomChangeCar", function(data, cb)
  if vehicle then
    DeleteEntity(vehicle)
  end
...
AddEventHandler("ucrp-showrooms-tuner:enterExperience", function()
  openShowroom("pdm")
...
RegisterNetEvent('ucrp-tunerlog')
AddEventHandler('ucrp-tunerlog', function(name) 
            local rank = exports["isPed"]:GroupRank("tuner_carshop")
            if rank > 4 then 
              TriggerEvent("ucrp-showrooms-tuner:enterExperience")
            else 
              TriggerEvent("DoShortHudText", "You don't work here!", 2)
    end
end)
