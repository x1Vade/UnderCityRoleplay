...
function HarnessOnCar()
    local ped = PlayerPedId()
    local vehicle = GetVehiclePedIsIn(ped, false)
    local plate = GetVehicleNumberPlateText(vehicle)
    if IsPedInAnyVehicle(PlayerPedId(), false) and harness then
  return harnessDurability end
  Citizen.Wait(5000)
       print('Harness : ' ,harnessDurability)
end
...
function toggleHarness()
   ...
   -- disableTurning()
    local finished = 0
    if harness then
        finished = exports["ucrp-taskbar"]:taskBar(1000,"Taking off Harness",true)
    else
        finished = exports["ucrp-taskbar"]:taskBar(3000,"Putting on Harness",true)
    end
    if (finished == 100) then
