...
AddEventHandler("vehicleMod:getHarness", function(plate)
    ...
    exports.ghmattimysql:execute("SELECT harness FROM characters_cars WHERE harness = @harness", {['harness'] = harness}, function(result)
...
AddEventHandler("vehicleMod:applyHarness", function(plate, durability)
    ...
    exports.ghmattimysql:execute("UPDATE characters_cars SET harness = @durability WHERE harness = @harness",
    {
        ['harness'] = harness,
...
AddEventHandler("vehicleMod:getHarness", function(plate, durability)
    ...
    if harnessChanges[harness] ~= nil then
        harness = harnessChanges[harness]
    end
