...
RegisterNetEvent('ucrp-housing:spawnpoints')
AddEventHandler('ucrp-housing:spawnpoints', function(spawns)
...
RegisterNetEvent('ucrp-housing:unlock&lockProperty', function()
...
AddEventHandler('housing:info:realtor', function(args)
	if modifying then
		...
		if args.action == "update" then
			...
			TriggerServerEvent("ucrp-housing:updatespawns",modifyingTable, modifyingTable["house_id"])
			return
		end
	end
	...
	if args.action == "PropertyOutstanding" then
		TriggerServerEvent("ucrp-housing:PropertyOutstanding",houseinfo[1],houseinfo[2])
	end
...
AddEventHandler('housing:transferHouseAttempt', function(cid)
	if cid ~= nil then
		if modifying then
			TriggerServerEvent("ucrp-housing:transferHouse",modifyingTable["house_id"],modifyingTable["house_model"],cid)
		else
...
AddEventHandler('housing:evictHouseAttempt', function()
	if modifying then
		TriggerServerEvent("ucrp-housing:evictHouse",modifyingTable["house_id"],modifyingTable["house_model"],exports["isPed"]:isPed("cid"))
	else
	...
	while true do
		...
		if backdooroutside["x"] ~= 0.0 and not insidehouse then
			...
			if #(vector3(backdooroutside["x"],backdooroutside["y"],backdooroutside["z"]-0.3) - plyCoords) < 1.5 then
				...
				if IsControlJustReleased(2, 38) then
					TriggerServerEvent("ucrp-housing:enterhousebackdoor", house_poi["house_id"], house_poi["house_model"], backdoorinside["x"],backdoorinside["y"],backdoorinside["z"])
					Citizen.Wait(3000)
				end
			end	
		...
		if clothing["x"] ~= 0.0 and insidehouse then
			if #(vector3(clothing["x"],clothing["y"],clothing["z"]-0.3) - plyCoords) < 1.5 then
				...
				TriggerEvent("ucrp-clothingmenu:enable", true)
				if IsControlJustReleased(1,Controlkey["housingSecondary"][1]) then
					logout()
				end
			else
				TriggerEvent("ucrp-clothingmenu:enable", false)
			end
...
AddEventHandler('housing:exit:backdoor', function(x,y,z,h)
	...
	TriggerEvent("ucrp-weathersync:client:EnableSync")
	myhouseid = 0
	myhousetype = 0
	entryType = 0
end)
...
AddEventHandler('housing:findsalecid', function(checkcid,price,house_id,house_model,storage,clothing,garages)
	...
	if buy then
		TriggerEvent("DoLongHudText","Attempting purchase - please wait.",91)
			TriggerServerEvent("ucrp-housing:purchasehouse",tonumber(checkcid),house_id,house_model,upfront,price,housename,storage,clothing,garages,false)
	end
...
AddEventHandler('housing:attemptpay', function()
	...
	TriggerServerEvent("ucrp-housing:dopayment", house_id, house_model)
...
AddEventHandler('housing:enter', function()
	...
	if modifying then
		TriggerServerEvent("ucrp-housing:enterhouse",exports['isPed']:isPed('cid'), house_id,house_model,true)
		housenameraid = house_name
	else
		TriggerServerEvent("ucrp-housing:enterhouse",exports['isPed']:isPed('cid'), house_id,house_model,false)
		housenameraid = "none"
	end
...
RegisterNetEvent("ucrp-housing:entersuccess")
AddEventHandler("ucrp-housing:entersuccess", function(house_id,house_model,furniture, x,y,z)
	...
	if rentedOffices[tonumber(house_id)] then
		if (rentedOffices[tonumber(house_id)]["trap"] ~= nil) then
			DoScreenFadeOut(1)
			TriggerEvent("ucrp-housing:entering")
		end
	end
	...
	TriggerEvent('ucrp-weathersync:client:DisableSync')
...
AddEventHandler('houses:GiveKey', function()
	...
	if(distance ~= -1 and distance < 5) then
		TriggerServerEvent("ucrp-housing:givekey",house_id,house_model,house_name,GetPlayerServerId(t))
...
AddEventHandler('houses:retrieveHouseKeys', function()
	...
	TriggerServerEvent("ucrp-housing:retrieveKeys",house_id,house_model)	
end)
...
AddEventHandler('houses:removeHouseKey', function(target)
	...
	TriggerServerEvent("ucrp-housing:removeKey",house_id, house_model, target)		
end)
