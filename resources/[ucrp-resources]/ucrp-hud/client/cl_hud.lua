...
RegisterNetEvent("ucrp-hud:EnableHud")
AddEventHandler("ucrp-hud:EnableHud", function(status)
    isLoggedIn = status
end)
...
local setdbOnOff = true
local setwindOnOff = true
local seterrorOnOff = true
local setlightOnOff = true
local setdollarOnOff = true
local setswimmingOnOff = true
local setweightOnOff = true
local settoxicOnOff = true
local developer = false
local debug = false
...
RegisterNetEvent('ucrp-admin:currentDevmode')
AddEventHandler('ucrp-admin:currentDevmode', function(test)
    developer = test
end)

RegisterNetEvent('ucrp-admin:currentDebug')
AddEventHandler('ucrp-admin:currentDebug', function(status)
    debug = status
end)
...
Citizen.CreateThread(function()
    ...
    while true do 
        ...
        if isLoggedIn then
            ...
            if weapon ~= `WEAPON_UNARMED` then shoot = true else shoot = false end
            ...
            SendNUIMessage({
                ...
                dev = developer,
                dev2 = debug,
                nitro = exports["ucrp-nitro"]:HowmuchNitro(), 
                casino = tiempo,
                inVehicle = inveh,
                cinturon = exports["ucrp-oVehicleMod"]:HarnessOnCar(),
...
RegisterCommand('hud', function()
    TriggerEvent('ucrp-hud:open-hud')
end)

RegisterNetEvent('ucrp-hud:open-hud')
AddEventHandler('ucrp-hud:open-hud', function()
...
local dbDisabled = false
RegisterNetEvent("client:disabledb")
AddEventHandler("client:disabledb",function(dbNew)
	dbDisabled = dbNew
end)

local windDisabled = false
RegisterNetEvent("client:disablewind")
AddEventHandler("client:disablewind",function(windNew)
	windDisabled = windNew
end)

local errorDisabled = false
RegisterNetEvent("client:disableerror")
AddEventHandler("client:disableerror",function(errorNew)
	errorDisabled = errorNew
end)

local lightDisabled = false
RegisterNetEvent("client:disablelight")
AddEventHandler("client:disablelight",function(lightNew)
	lightDisabled = lightNew
end)

local dollarDisabled = false
RegisterNetEvent("client:disabledollar")
AddEventHandler("client:disabledollar",function(dollarNew)
	dollarDisabled = dollarNew
end)

local swimmingDisabled = false
RegisterNetEvent("client:disableswimming")
AddEventHandler("client:disableswimming",function(swimmingNew)
	swimmingDisabled = swimmingNew
end)

local weightDisabled = false
RegisterNetEvent("client:disableweight")
AddEventHandler("client:disableweight",function(weightNew)
	weightDisabled = weightNew
end)

local toxicDisabled = false
RegisterNetEvent("client:disabletoxic")
AddEventHandler("client:disabletoxic",function(toxicNew)
	toxicDisabled = toxicNew
end)
...
RegisterNetEvent('ucrp-gas:checkpump')
AddEventHandler('ucrp-gas:checkpump', function()
...
RegisterNetEvent('ucrp-admin:maxstats')
AddEventHandler('ucrp-admin:maxstats', function()
    currentValues["thirst"] = 100
	currentValues["hunger"] = 100 
    TriggerEvent("heal", PlayerPedId())
    TriggerEvent("Hospital:HealInjuries", PlayerPedId(),true) 
    TriggerServerEvent("ucrp-death:reviveSV", source)
...
RegisterNetEvent("client:updatedb")
AddEventHandler("client:updatedb",function(newdb)
    pdb = newdb
end)

RegisterNetEvent("client:updatewind")
AddEventHandler("client:updatewind",function(newwind)
    pwind = newwind
end)

RegisterNetEvent("client:updateerror")
AddEventHandler("client:updateerror",function(newerror)
    perror = newerror
end)

RegisterNetEvent("client:updatelight")
AddEventHandler("client:updatelight",function(newlight)
    plight = newlight
end)

RegisterNetEvent("client:updatedollar")
AddEventHandler("client:updatedollar",function(newdollar)
    pdollar = newdollar
end)

RegisterNetEvent("client:updateswimming")
AddEventHandler("client:updateswimming",function(newswimming)
    pswimming = newswimming
end)

RegisterNetEvent("client:updateweight")
AddEventHandler("client:updateweight",function(newweight)
    pweight = newweight
end)

RegisterNetEvent("client:updatetoxic")
AddEventHandler("client:updatetoxic",function(newtoxic)
    ptoxic = newtoxic
end)
...
RegisterNetEvent("ucrp-admin:currentDevmode")
AddEventHandler("ucrp-admin:currentDevmode", function(devmode)
    isBlocked = devmode
end)
...
RegisterNetEvent("ucrp-hud:updateStress")
AddEventHandler("ucrp-hud:updateStress", function(status, amount)
    if status == true then
...
RegisterCommand('ui-r', function()
    ...
    TriggerEvent('ucrp-hud:EnableHud', false)
    TriggerEvent('closeInventoryGui')
    TriggerEvent('ucrp-banking:ResetUI')
    TriggerEvent('ucrp-radio:ResetUI')
    TriggerEvent('ucrp-menu:ResetUI')
    TriggerEvent('ucrp-interact:ResetUI')
    TriggerEvent('ucrp-mdt:ResetUI')
    TriggerEvent('phone:reset')
    TriggerEvent('clothing:close')
    exports['ucrp-interaction']:hideInteraction()
    TriggerEvent('closeinteraction')
    Wait(1000)
    
    --exports['ucrp-interaction']:showInteraction("UI Successfully Restarted")
    --Wait(1000)
    exports['ucrp-interaction']:hideInteraction()
...
function removeAttachedProp2()
  ...
  RegisterNetEvent("ucrp-hud:jerry_can:refuel")
  AddEventHandler("ucrp-hud:jerry_can:refuel",function()
      vehicleCurrentFuel = DecorGetInt(veh, "GetVehicleCurrentFuel")
      if exports['ucrp-inventory']:hasEnoughOfItem('883325847', 1) then
        local timer = (100 - vehicleCurrentFuel) * 400
        refillVehicle()
        local finished = exports["ucrp-taskbar"]:taskBar(timer,"Refueling")
        local veh = getVehicleClosestToMe()
