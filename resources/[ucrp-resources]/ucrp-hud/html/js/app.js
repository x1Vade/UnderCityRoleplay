...
let setdbOnOff = true;
let setwindOnOff = true;
let seterrorOnOff = true;
let setlightOnOff = true;
let setdollarOnOff = true;
let setswimmingOnOff = true;
let setweightOnOff = true;
let settoxicOnOff = true;
...
(() => {
    ...
    NPUI.UpdateHud = function(data) {
        ...
        if (!setdbOnOff) {
            ...
            if (data.db >= 3) {
                $('.hdb').fadeIn(5000);
                $('.fa-database').fadeIn(5000);
            }
            if (data.db <= 2) {
                $('.hdb').fadeOut(5000);
                $('.fa-database').fadeOut(5000);
            }
        }
        Progress(data.wind, ".wind");
        if (!setwindOnOff) {
            $('.hwind').hide();
            $('.fa-wind').hide();
        } else {
        ...
        if (!seterrorOnOff) {
            ...
            if (data.error >= 3) {
                $('.herror').fadeIn(5000);
                $('.fa-exclamation').fadeIn(5000);
            }
            if (data.error <= 2) {
                $('.herror').fadeOut(5000);
                $('.fa-exclamation').fadeOut(5000);
            }
        }
        ...
        if (!setlightOnOff) {
            ...
            if (data.light >= 3) {
                $('.hlight').fadeIn(5000);
                $('.fa-lightbulb').fadeIn(5000);
            }
            if (data.light <= 2) {
                $('.hlight').fadeOut(5000);
                $('.fa-lightbulb').fadeOut(5000);
            }
        }
        ...
        if (!setdollarOnOff) {
            ...
            if (data.dollar >= 3) {
                $('.hdollar').fadeIn(5000);
                $('.fa-dollar-sign').fadeIn(5000);
            }
            if (data.dollar <= 2) {
                $('.hdollar').fadeOut(5000);
                $('.fa-dollar-sign').fadeOut(5000);
            }
        }
        ...
        if (!setswimmingOnOff) {
            ...
            if (data.swimming >= 3) {
                $('.hswimming').fadeIn(5000);
                $('.fa-water').fadeIn(5000);
            }
            if (data.swimming <= 2) {
                $('.hswimming').fadeOut(5000);
                $('.fa-water').fadeOut(5000);
            }
        }
        Progress(data.weight, ".weight");
        if (!setweightOnOff) {
            $('.hweight').hide();
            $('.fa-dumbbell').hide();
        } else {
            if (data.weight >= 3) {
                $('.hweight').fadeIn(5000);
                $('.fa-dumbbell').fadeIn(5000);
            }
            if (data.weight <= 2) {
                $('.hweight').fadeOut(5000);
                $('.fa-dumbbell').fadeOut(5000);
            }
        }
        Progress(data.toxic, ".toxic");
        if (!settoxicOnOff) {
            $('.htoxic').hide();
            $('.fa-biohazard').hide();
        } else {
            if (data.toxic >= 3) {
                $('.htoxic').fadeIn(5000);
                $('.fa-biohazard').fadeIn(5000);
            }
            if (data.toxic <= 2) {
                $('.htoxic').fadeOut(5000);
                $('.fa-biohazard').fadeOut(5000);
            }
        }
        ...
        if (data.cinturon >= 0 && data.inVehicle) {
        ...
        if (data.shoot) {
            $('.hshootmode').fadeIn(500, function() { $(".hshootmode").css("display", "flex"); });
            $('.fa-stream').fadeIn(500, function() { $(".hshootmode").css("display", "flex"); });
        } else {
            $('.hshootmode').fadeOut(500, function() { $(".hshootmode").css("display", "none"); });
            $('.fa-stream').fadeOut(500, function() { $(".hshootmode").css("display", "none"); });
        }
            ...
            switch(event.data.action) {
                ...
                case 'showMenu':
                    ...
                    $('#hud-on').on('change',function(){
                        ...
                        $.post('https://ucrp-hud/hud_on',JSON.stringify({
                            hud_on: hud_on
                        }));
                    });
                    ...
                    $('#db-on').on('change',function(){
                        setdbOnOff = this.checked
                    });
                    $('#wind-on').on('change',function(){
                        setwindOnOff = this.checked
                    });
                    $('#error-on').on('change',function(){
                        seterrorOnOff = this.checked
                    });
                    $('#light-on').on('change',function(){
                        setlightOnOff = this.checked
                    });
                    $('#dollar-on').on('change',function(){
                        setdollarOnOff = this.checked
                    });
                    $('#swimming-on').on('change',function(){
                        setswimmingOnOff = this.checked
                    });
                    $('#weight-on').on('change',function(){
                        setweightOnOff = this.checked
                    });
                    $('#toxic-on').on('change',function(){
                        settoxicOnOff = this.checked
                    });
                    ...
                    $('#compassinput').on('change',function(){
                        ...
                        $.post('https://ucrp-hud/compassinput',JSON.stringify({
                    ...
                    $('#compassTime').on('change',function(){
                        ...
                        $.post('https://ucrp-hud/compassTime',JSON.stringify({
                    ...
                    $('#compassStreet').on('change',function(){
                        ...
                        $.post('https://ucrp-hud/compassStreet',JSON.stringify({
                    ...
                    $('#d_minimap').on('change',function(){
                        ...
                        $.post('https://ucrp-hud/minimap_clip',JSON.stringify({
                            d_minimap: d_minimap
                        }));
                    });
                    ...
                    $('#speedmeter').keyup(function(){
                        ...
                        $.post('https://ucrp-hud/update:timer:speed',JSON.stringify({
                            fps_speed: fps_speed
                        }));
                    });
...
$('#minimap').change(function(){
    let on = this.checked
    $.post('https://ucrp-hud/ayar-minimap',JSON.stringify({
        minimap: on
    }));
});
...
ronin.CloseAyarMenu = function() {
    ...
    $.post('https://ucrp-hud/close-ayar-menu');
};
