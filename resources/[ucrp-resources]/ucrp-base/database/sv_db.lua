...
function NPX.DB.CreateNewCharacter(self, src, data2, hexid, ph, callback)
    ...
    exports.ghmattimysql:execute(q, v, function(rowsChanged)
        ...
        -- exports["ucrp-financials"]:BankAccountCreation(qData.instertId, "Personal Account", 1, 5000, 31, 0, 0, 1, false)
        -- print(("[Spriest] Created new Character for %s"):format(hexid))
        -- print(("[Spriest] Created new bank account for %s(CID)"):format(qData.insertId))
    end)
end
...
function NPX.DB.UpdateCharacterStressLevel(self, user, characterId, newLevel, callback)
    ...
    exports["ucrp-log"]:AddLogHex("Base", 
        tostring(hexId), 
        "Update Character Stress Level", 
        { hexId = hexId, characterId = characterId, newLevel = newLevel })
    exports.ghmattimysql:execute(q, v, function(rowsChanged)
...
callback = callback and callback or function() return end
    ...
    exports["ucrp-log"]:AddLogHex("Base", 
        tostring(hexId), 
        "Update Character Dirty Money", 
        { hexId = hexId, characterId = characterId, amount = amount })
...
function NPX.DB.UpdateControls(self, src, controlsTable, callback)
    callback = callback and callback or function() return end
        ...
        exports["ucrp-log"]:AddLogHex("Base", 
            ...
            { hexId = hexId, controls = json.encode(controlsTable) })
...
function NPX.DB.UpdateSettings(self, src, settingsTable, callback)
    ...
    local user = exports["ucrp-base"]:getModule("Player"):GetUser(src)
        ...
        exports["ucrp-log"]:AddLogHex("Base", 
            ...
            { hexId = hexId, controls = json.encode(settingsTable) })
