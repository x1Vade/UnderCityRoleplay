        ...
        if vehModel ~= nil and vehModel ~= 0 then
            if DoesEntityExist(currVeh) then
                if IsThisModelABoat(vehModel) or IsThisModelAJetski(vesModel) or IsThisModelAnAmphibiousCar(vehModel) or IsThisModelAnAmphibiousQuadbike(vehModel) then
                	local finished = exports["ucrp-taskbar"]:taskBar(2000,"Toggling Anchor")
...
AddEventHandler('rich:TakePlayers', function(count)
    ...
    if SetDiscordRichPresenceAction then
        SetDiscordRichPresenceAction(0, 'Discord', 'https://discord.gg/tKTtjXQcCT')
    end
end)
...
Citizen.CreateThread(function()
    ...
    while true do
        Citizen.Wait(2000)
        SetDiscordRichPresenceAssetText("https://discord.gg/tKTtjXQcCT")
        SetRichPresence(onlinePlayers.."/64 | Poppin Mints")
