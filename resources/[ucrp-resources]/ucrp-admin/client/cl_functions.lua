...
function ToggleDevMode(Bool)
    TriggerEvent('ucrp-admin:currentDevmode', Bool)
    devmodeToggle = Bool
end
...
function RefreshMenu(Type)
    if Type == 'Favorites' then
        -- Favorites
        if GetResourceKvpString("ucrp-adminmenu-favorites") == nil or GetResourceKvpString("ucrp-adminmenu-favorites") == "[]" then
            Config.FavoritedItems = GenerateFavorites()
            SetResourceKvp("ucrp-adminmenu-favorites", json.encode(Config.FavoritedItems))
        else
            Config.FavoritedItems = json.decode(GetResourceKvpString("ucrp-adminmenu-favorites"))
        end
    elseif Type == 'Targets' then
        if GetResourceKvpString("ucrp-adminmenu-pinned_targets") == nil or GetResourceKvpString("ucrp-adminmenu-pinned_targets") == "[]" then
            Config.PinnedTargets = GeneratePinnedPlayers()
            SetResourceKvp("ucrp-adminmenu-pinned_targets", json.encode(Config.PinnedTargets))    
        else
            Config.PinnedTargets = json.decode(GetResourceKvpString("ucrp-adminmenu-pinned_targets"))
        end
    elseif Type == 'Settings'then
        if GetResourceKvpString("ucrp-adminmenu-settings") == nil or GetResourceKvpString("ucrp-adminmenu-settings") == "[]" then
            Config.AdminSettings = GenerateAdminSettings()
            SetResourceKvp("ucrp-adminmenu-settings", json.encode(Config.AdminSettings))
        else
            Config.AdminSettings = json.decode(GetResourceKvpString("ucrp-adminmenu-settings"))
        end
...
function GetPlayersInArea(Coords, Radius)
	...
	RPC.execute('ucrp-admin/server/get-active-players-in-radius', function(Players)
...
function GetPlayers()
    ...
    local Players = RPC.execute("ucrp-admin/server/get-players")
...
AvailableGangs = {}
function GetGangs()
    local data = exports["ucrp-gangsnew"]:GetAvailableGangs()
...
AddEventHandler("ucrp-admin:noClipToggle", function(pIsEnabled)
...
RegisterNetEvent('ucrp-admin:OpenMenuBind', function()
    if not devmodeToggle then return end
    TriggerEvent('ucrp-admin-AttemptOpen',true)
end)

RegisterNetEvent('ucrp-admin:NoClipBind', function()
    if not devmodeToggle then return end
    local bool = not isInNoclip
    RunNclp(nil,bool)
    TriggerEvent("ucrp-admin:noClipToggle",bool)
end)

RegisterNetEvent('ucrp-admin:Teleport', function()
  if not devmodeToggle then return end
  teleportMarker(nil)
end)

RegisterNetEvent("ucrp-admin:ReviveInDistance")
AddEventHandler("ucrp-admin:ReviveInDistance", function()
    local playerList = {}
...
RegisterNetEvent("ucrp-adminmenu:runSpawnCommand")
AddEventHandler("ucrp-adminmenu:runSpawnCommand", function(model, livery)
...
RegisterNetEvent("ucrp-admin:dv")
AddEventHandler("ucrp-admin:dv", function()
