...
function dwStatusHud(pValues)
  if dwRefreshChipCount then
    ...
    dwCurrentChipCount = RPC.execute("ucrp-casino:getCurrentChipCount")
  end
...
Citizen.CreateThread(function()
  --Wait(10000)
  dealerPedSeeds = RPC.execute("ucrp-blackjack:getDealerPedSeeds")
    ...
    while true do 
        if not sittingAtBlackjackTable then
            if closestChair ~= nil and closestChairDist < 1.6 then
                if not timeoutHowToBlackjack then
                    if not uiIntShown then
                        ...
                        if exports['ucrp-inventory']:hasEnoughOfItem("casinomember", 1, false, true) and not isHighRoller then
                            background = "success"
                        ...
                        exports["ucrp-ui"]:showInteraction(
            ...
            elseif uiIntShown then
                uiIntShown = false
                local background = "info"
                if exports['ucrp-inventory']:hasEnoughOfItem("casinomember", 1, false, true) or exports['ucrp-inventory']:hasEnoughOfItem("casinovip", 1, false, true) then
                    background = "success"
                else
                    background = "error"
                end
                exports["ucrp-ui"]:hideInteraction(background)
            end
        elseif uiIntShown then
            uiIntShown = false
            local background = "info"
...
RegisterUICallback("ucrp-ui:blackjackHitStand", function(data, cb)
  ...
  elseif action == "double" then
    local dwUseChipsResult = RPC.execute("ucrp-casino:useChips", currentBetAmount)
  ...
  elseif action == "surrender" then
    ...
    -- RPC.execute("ucrp-casino:winChips", currentBetAmount / 2)
...
AddEventHandler("ucrp-ui:application-closed", function(name)
...
Citizen.CreateThread(function()
    while true do
        ...
        --         data[#data + 1] = {
        --           action = "ucrp-ui:blackjackHitStand",
...
RegisterUICallback("ucrp-ui:blackJackBet", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
...
AddEventHandler("ucrp-ui:application-closed", function(name, data)
...
AddEventHandler("Blackjack:blackjackPush",function(tableId)
    -- RPC.execute("ucrp-casino:winChips", currentBetAmount)
    dwRefreshChipCount = true
    if closeToCasino then
        blackjackGameInProgress = false
        dealerPed = getDealerFromTableId(tableId)
...
AddEventHandler("Blackjack:blackjackWin",function(tableId, dwHitBlackjack)
    ...
    -- RPC.execute("ucrp-casino:winChips", (currentBetAmount * 2) + extra)
    dwRefreshChipCount = true
    if closeToCasino then
        blackjackGameInProgress = false
        dealerPed = getDealerFromTableId(tableId)
