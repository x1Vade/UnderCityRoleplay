    ...
    exports.ghmattimysql:execute("SELECT id, paycheck FROM characters WHERE id = @id", {
        ...
        if result[1] ~= nil then
            ...
            if curPaycheck > 0 then
                ...
                local targetAccountId = exports["ucrp-financials"]:getDefaultBankAccount(cid, true)
                if targetAccountId == false or type(targetAccountId) == "string" then
                    TriggerClientEvent("DoLongHudText", src, "Can't transfer your paycheck, seems like it's frozen.", 2)
                else
                    exports["ucrp-financials"]:DoPersonalIncomeTransaction(src, 1, targetAccountId, curPaycheck, "Paycheck", "payslip")
