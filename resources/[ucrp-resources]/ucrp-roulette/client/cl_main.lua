...
createRulettAsztal = function(index, data)
    ...
    self.tableObject = GetHashKey('vw_prop_casino_roulette_01'), data.position, false
...
RegisterNetEvent("roulette:sit:down", function()
        while true do
            local letSleep = true

            local playerpos = GetEntityCoords(PlayerPedId())

            if closetoRulett then
...
Citizen.CreateThread(function()
    exports["ucrp-polytarget"]:AddBoxZone("roulette_table1", vector3(1025.62, 47.77, 70.81), 3.0, 3.0, {
        heading=0,
        minZ=68,
        maxZ=73,
        debugPoly = true 
...
AddEventHandler("ucrp-polyzone:enter", function(zone, data)
    if zone == "roulette_table1" then 
        roulette = true
        closetoRulett = true
        TriggerEvent("roulette:sit:down")
        exports["ucrp-ui"]:showInteraction("[E] Play Roulette")
    end
end)
