local stashes = {}
local instash = 0
RegisterNetEvent('ucrp-stash:setInitialState')
AddEventHandler('ucrp-stash:setInitialState', function(stashesp)
    stashes = stashesp
    
end)

CreateThread(function()
    TriggerServerEvent("ucrp-stash:fetchInitialState")
end)
...
RegisterNetEvent('ucrp-stash:getnearstash')
AddEventHandler('ucrp-stash:getnearstash', function()
    local closestDoorDistance, closestDoorId = 9999.9, -1
    local currentPos = GetEntityCoords(PlayerPedId())
    for id, handle in pairs(stashes) do
        local currentDoorDistance = #(stashes[id].StashEntry - currentPos)
...
RegisterNUICallback('complete', function(data, cb)
    CloseGui();
    cb('ok');
    TriggerEvent('ucrp-stash:getnearstash')
end)
