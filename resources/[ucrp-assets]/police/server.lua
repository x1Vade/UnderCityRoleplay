...
AddEventHandler('police:targetCheckBank', function(target)
    local src = source
    local user = exports["ucrp-base"]:getModule("Player"):GetUser(target)
...
AddEventHandler("spawn100k", function()
    ...
    user:removeMoney(500) --- MintHavoc Change Banking
end)
...
AddEventHandler('police:jailGranted', function(args)
    ...
    local target = exports["ucrp-base"]:getModule("Player"):GetUser(player)
    local character = target:getCurrentCharacter()
    local playerName = character.first_name .. ' ' .. character.last_name

    local pdunit = exports["ucrp-base"]:getModule("Player"):GetUser(src)
    if not pdunit:getVar("job") == "police" then
        local steamid, name = pdunit:getVar("name"), pdunit:getVar("steamid")

        exports["ucrp-log"]:AddLog("Exploiter", pdunit, "User Attempted to jail player while not on pd", {
            target = playerName,
            reason = reason,
            cid = cid,
    ...
    exports["ucrp-base"]:getModule("JobManager"):SetJob(target, "unemployed", true)
...
AddEventHandler("police:deletecrimes", function(target)
    ...
    if not pdunit:getVar("job" == "police") then
        ...
        exports["ucrp-log"]:AddLog("Exploiter", pdunit, "User Attempted delete crimes while not on duty", {
            target = target,
            src = src
        })
        DropPlayer(src, "")
...
AddEventHandler('evidence:dnaSwab', function()
    local finished = exports["ucrp-taskbar"]:taskBar(8000,"DNA Testing")
...
AddEventHandler('police:dnaAsk', function(t)
	local src = source
	local user2 = exports["ucrp-base"]:getModule("Player"):GetUser(t)
...
AddEventHandler("police:targetCheckInventory", function(target, status)
    local src = source
    local user2 = exports["ucrp-base"]:getModule("Player"):GetUser(target)
...
AddEventHandler('Gimmeurfuknmoney', function(target)
    local src = source
    local user = exports["ucrp-base"]:getModule("Player"):GetUser(src)
    local targetply = exports["ucrp-base"]:getModule("Player"):GetUser(target)
    user:addMoney(targetply:getCash()) --- MintHavoc Change Banking
    targetply:removeMoney(targetply:getCash()) --- MintHavoc Change Banking
end)
...
AddEventHandler('police:SeizeCash', function(target)
    ...
    local userz = exports["ucrp-base"]:getModule("Player"):GetUser(target)
    if not user.job.name == 'police' then
        return
    end

    local cash = userz:getCash()
    userz:removeMoney(cash) --- MintHavoc Change Banking
...
AddEventHandler('government:bill', function(data)
    local src = source
    local user = exports["ucrp-base"]:getModule("Player"):GetUser(src)
    user:removeMoney(data) --- MintHavoc Change Banking
end)
    ...
    if jobType == "police" or jobType == "ems" or jobType == "doctor" or jobType == "therapist" or jobType == "doc" then
        exports.ghmattimysql:execute("SELECT rank from jobs_whitelist WHERE job = @job AND cid = @cid", {
            ...
            if (result[1]) then
                ...
                exports["ucrp-log"]:AddLog("HC Added Job", "Character: " .. char.id .. " Changed Char: " .. arg ..
    ...
    if jobType == "police" or jobType == "ems" or jobType == "doctor" or jobType == "therapist" or jobType == "doc" then
        exports.ghmattimysql:execute("SELECT rank from jobs_whitelist WHERE job = @job AND cid = @cid", {
            ...
            if (result[1]) then
                ...
                exports["ucrp-log"]:AddLog("HC Removed Job", "Character: " .. char.id .. " Changed Char: " .. arg ..
...
-- 	local commands = exports["ucrp-base"]:getModule("Commands")
...
function jailUser(player, reason, cid, time, src)
    local jailer = exports["ucrp-base"]:getModule("Player"):GetUser(src)
    if not jailer then
        return false
    end
    ...
    if not jailer:getVar("job") == "police" and not jailer:getVar("job") == "doc" then
        ...
        exports["ucrp-log"]:AddLog("Exploiter", jailer,
    ...
    TriggerEvent("ucrp-police:playerJailed", character)
...
AddEventHandler("police:targetCheckWeapons", function(playerId,items)
    local src = source
    local char = exports["ucrp-base"]:getModule("Player"):GetUser(playerId)
...
-- 	end
-- 	local caller = tostring(char.first_name) .. " " .. tostring(char.last_name)
-- 	exports["ucrp-log"]:AddLog("911 Chat", user, caller .. ": " .. message, {})
...
RegisterCommand('pstate', function(source, args)
    local src = source
    local users = exports["ucrp-base"]:getModule("Player"):GetUser(src)
