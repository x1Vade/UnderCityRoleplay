...
Citizen.CreateThread(function()
    exports["ucrp-keybinds"]:registerKeyMapping("", "Player", "Tackle", "+plyTackel", "-plyTackel")
...
Citizen.CreateThread(function()
    while true do
        ...
        TriggerServerEvent("ucrp-base:updatecoords", coords.x, coords.y, coords.z)
        Wait(13000)
    end
end)
    ...
    while true do
        ...
        if recentsearch == 0 then
            ...
            if endDistance < 0.7 or endDistance2 < 0.7 then
                ...
                currentWeapons = exports["ucrp-inventory"]:GetCurrentWeapons()
...
AddEventHandler("police:rob", function()
    ...
    if not exports["isPed"]:isPed("dead") then
        ...
        local finished = exports["ucrp-taskbar"]:taskBar(3500, "Robbing", false, true)
...
AddEventHandler('KneelHU', function()
    ...
    if (DoesEntityExist(player) and not IsEntityDead(player)) then
        ...
        local finished = exports["ucrp-taskbar"]:taskBar(2500, "Surrendering")
...
AddEventHandler('police:wshotgun', function()
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.0 then
            ...
            local finished = exports["ucrp-taskbar"]:taskBar(2000, "Unracking Shotgun")
            if finished == 100 then
...
AddEventHandler('police:wrifle', function()
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.0 then
            ...
            loadAnimDict('anim@narcotics@trash')
            TaskPlayAnim(PlayerPedId(), 'anim@narcotics@trash', 'drop_front', 0.9, -8, 1900, 49, 3.0, 0, 0, 0)
            local finished = exports["ucrp-taskbar"]:taskBar(2000, "Unracking Rifle")
            if finished == 100 then
...
AddEventHandler('police:woxy', function()
    ...
    if vehFront > 0 then
        ...
        local finished = exports["ucrp-taskbar"]:taskBar(4000, "Grabbing Scuba Gear")
        if finished == 100 then
            loadAnimDict('anim@narcotics@trash')
            TaskPlayAnim(PlayerPedId(), 'anim@narcotics@trash', 'drop_front', 0.9, -8, 1900, 49, 3.0, 0, 0, 0)
...
AddEventHandler('giveRadioToPed', function()
    ...
    if (not exports["ucrp-inventory"]:hasEnoughOfItem("radio", 1, false)) then
...
AddEventHandler('swatcopLoadout', function()
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.0 then
            ...
            local finished = exports["ucrp-taskbar"]:taskBar(2500, "Changing to Swat")
            if finished == 100 then
                ChangeToSkinNoUpdatePolice("s_m_y_swat_01")
            end
            ClearPedTasks(PlayerPedId())
...
AddEventHandler('police:gsr', function(t)
    ...
    local finished = exports["ucrp-taskbar"]:taskBar(15000, "GSR Testing")
    if finished == 100 then
        t, distance = GetClosestPlayer()
        if (distance ~= -1 and distance < 7) then
...
-- local finished = 0
-- if not isDead then
-- finished = exports["ucrp-taskbarskill"]:taskBar(1200,7)
-- end
...
AddEventHandler('police:getArrested2', function(cuffer)
    ...
    if not isDead and cuffAttemptCount < 4 then
        ...
        finished = exports["ucrp-ui"]:taskBarSkill(cuffAttemptTbl[cuffAttemptCount][1],
...
AddEventHandler('police:jailing', function(time)
    Citizen.Trace("Jailing in process.")
    TriggerServerEvent('police:jailGranted', time)
    TriggerServerEvent('updateJailTime', tonumber(time[2]))
end)
    ...
    if t ~= nil and t ~= -1 then
        if (distance ~= -1 and distance < 5) then
            ...
            if myjob ~= "ems" and myjob ~= "doctor" then
                local bandages = exports["ucrp-inventory"]:getQuantity("bandage")
                if bandages == 0 then
                    return
                else
...
AddEventHandler("ems:stomachpump", function()
    ...
    if t ~= nil and t ~= -1 then
        if (distance ~= -1 and distance < 5) then
            local finished = exports["ucrp-taskbar"]:taskBar(10000, "Inserting stomach pump 🤢", false, true)
...
function vehCruise()
	if GetLastInputMethod(2) then
		local isInVeh = IsPedInAnyVehicle(PlayerPedId(), true)
		if isInVeh then
			TriggerEvent("toggle:cruisecontrol")
		end
	end
...
Citizen.CreateThread(function()
exports["ucrp-binds"]:registerKeyMapping("", "Vehicle", "Cruise Control", "+vehCruise", "-vehCruise", "X")
RegisterCommand('+vehCruise', vehCruise, false)
RegisterCommand('-vehCruise', function() end, false)
end)

RegisterCommand(toggle:cruisecontrol)
...
AddEventHandler('FlipVehicle', function()
    ...
    local finished = exports["ucrp-taskbar"]:taskBar(18000, "Flipping Vehicle Over", false, true)
...
function deleteVeh(ent)
    ...
    local finished = exports["ucrp-taskbar"]:taskBar(1000, "Impounding", false, true)
...
local function serviceVehicle(arg, livery, isEmsWhiteListed, cb)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerEvent("ucrp-admin:runSpawnCommand", selectedSkin, livery)
    end)
end
...
AddEventHandler("police:chatCommand", function(args, cmd, isVehicleCmd)
    ...
    if isCop or isMedic then
        ...
        if cmd == "fix" then
            local finished = exports["ucrp-taskbar"]:taskBar(3500, "Completing Task", false, true)
...
AddEventHandler("police:dorifto", function()
    if (IsPedInAnyVehicle(PlayerPedId(), false)) then
        ...
        if Driver == PlayerPedId() and IsVehicleModel(veh, defaultHash) or IsVehicleModel(veh, defaultHash2) or
            ...
            local finished = exports["ucrp-taskbar"]:taskBar(3000, "Toggling Drift Mode", false, true)
...
AddEventHandler('police:frisk', function()
    t, distance = GetClosestPlayer()
    if (distance ~= -1 and distance < 5) then
        local items = exports["ucrp-inventory"]:getFullItemList()
        TriggerServerEvent("police:targetCheckWeapons", GetPlayerServerId(t),items)
        ExecuteCommand("e search")
        local finished = exports["ucrp-taskbar"]:taskBar(3000, "Frisking")
    else
        TriggerEvent("DoLongHudText", "No player near you!", 2)
    end
end)
