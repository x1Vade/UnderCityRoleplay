...
AddEventHandler("ucrp-inventory:itemUsed", function(pItem)
...
Citizen.CreateThread(function()
    while true do
        ...
        if statusS == true then
            TriggerServerEvent("ucrp-spawn:inspawn", true)
        end
    end
end)
...
function Save(save, close)
    ...
    if save then
        ...
        if not startingMenu or passedClothing then
            ...
            TriggerEvent("ucrp-spawn:finishedClothing","Finished")
        ...
        TriggerEvent("ucrp-spawn:finishedClothing","Old")
        LoadPed(oldPed)
...
AddEventHandler("ucrp-base:initialSpawnModelLoaded", function()
...
AddEventHandler("ucrp-polyzone:enter", function(zone, data)
    ...
    if currentZone then
        exports['ucrp-ui']:showInteraction('[M] '..MenuData[zone].text..'')
...
RegisterNetEvent("raid_clothes:attempt:change", function(data)
    local pNewName = exports["ucrp-applications"]:KeyboardInput({
