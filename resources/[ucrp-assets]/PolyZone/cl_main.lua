...
RegisterNetEvent('ucrp-base:PolyZoneUpdate')
AddEventHandler('ucrp-base:PolyZoneUpdate', function()
    local combo = ComboZone:Create({}, {name = "ucrp-polyzone", debugPoly = false})
    combo:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
        if enteredZones then
            for i = 1, #enteredZones do
                TriggerEvent('ucrp-polyzone:entered:'.. enteredZones[i].data.cComboZone, enteredZones[i].name, enteredZones[i].data)
                print('ucrp-polyzone:ssadasdsadsda2')
            end
        end
        if leftZones then
            for i = 1, #leftZones do
                TriggerEvent('ucrp-polyzone:exited:'.. leftZones[i].data.cComboZone, leftZones[i].name)
                print('ucrp-polyzone:stop2')
            end
        end
    end)
...
AddEventHandler("onResourceStart", function(resourceName)
    ...
    combo:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
        if enteredZones then
            for i = 1, #enteredZones do
                TriggerEvent('ucrp-polyzone:entered:'.. enteredZones[i].data.cComboZone, enteredZones[i].name, enteredZones[i].data)
                print('ucrp-polyzone:wedasdsadasdsadsadsa')
            end
        end
        if leftZones then
            for i = 1, #leftZones do
                TriggerEvent('ucrp-polyzone:exited:'.. leftZones[i].data.cComboZone, leftZones[i].name)
                print('ucrp-polyzone:stop')
            end
        end
