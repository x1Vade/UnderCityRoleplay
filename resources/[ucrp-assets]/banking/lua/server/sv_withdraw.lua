...
AddEventHandler('banking:server:Withdraw', function(account, amount, note, fSteamID)
    local src = source
    ...
    if not amount or tonumber(amount) <= 0 then
        TriggerClientEvent("banking:client:Notify", src, "error", "Invalid amount!") 
        return
    end
    ...
    if account == "personal" then
        if amount > Player.PlayerData.money["bank"] then
            TriggerClientEvent("banking:client:Notify", src, "error", "Your bank doesn't have this much money!") 
            return
        end
    ...
    if(account == "business") then
        ...
        if data then
            ...
            if sM >= amount then
                TriggerEvent('banking:society:server:WithdrawMoney',src, amount, data.name)
                ...
                TriggerClientEvent("banking:client:Notify", src, "error", "Not enough money current balance: €"..sM) 
            end
        end
    end
