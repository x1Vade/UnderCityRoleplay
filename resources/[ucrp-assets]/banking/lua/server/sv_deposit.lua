...
AddEventHandler('banking:server:Deposit', function(account, amount, note, fSteamID)
    local src = source
    ...
    if amount > Player.PlayerData.money["cash"] then
        TriggerClientEvent("banking:client:Notify", src, "error", "You can't afford this!") 
        return
    end
    ...
    if account == "business"  then
        ...
        if data then
            ...
            TriggerEvent('banking:society:server:DepositMoney', src, deposit, data.name)
