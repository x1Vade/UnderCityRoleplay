RegisterServerEvent('banking:server:Transfer')
AddEventHandler('banking:server:Transfer', function(target, account, amount, note, fSteamID)
    local src = source
    ...
    local targetPly = ucrp-base.Functions.GetPlayer(target)
    ...
    if (account == "business") then
        ...
        if data then
            ...
            TriggerEvent('banking:society:server:WithdrawMoney', src, amount, society)
            Wait(50)
