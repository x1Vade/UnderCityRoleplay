...
function openGui()
  ...
  elseif job == "ems" then
    Emergency = true
  elseif exports["ucrp-inventory"]:hasEnoughOfItem("scanner",1,false) then
    Emergency = true
  end
...
local function handleConnectionEvent(pChannel)
  ...
  if newChannel < 1.0 then
    ...
    exports['ucrp-voice']:addPlayerToRadio(newChannel)
  end
end
...
function hasRadio()
    if exports["ucrp-inventory"]:hasEnoughOfItem("radio", 1, false, true) or exports["ucrp-inventory"]:hasEnoughOfItem("civradio", 1, false, true)then
      return true
    else
      return false
    end
...
RegisterNUICallback('volumeUp', function(data, cb)
  ...
  TriggerEvent('ucrp-voice:SetRadioVolume', radioVolume)
...
AddEventHandler('ucrp-inventory:itemCheck', function (item, state, quantity)
    if item ~= "civradio" and item ~= "radio" and item ~="scanner" then return end
    if state or quantity > 0 then return end
    exports["ucrp-voice"]:removePlayerFromRadio(false)
    exports["ucrp-ui"]:sendAppEvent("radio", { value = 0, state = false })
