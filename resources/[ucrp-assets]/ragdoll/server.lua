...
RegisterServerEvent('ucrp-ragdoll:removeallitems')
AddEventHandler('ucrp-ragdoll:removeallitems', function()
    local src = source
...
AddEventHandler('trycpr', function()
	local user = exports["ucrp-base"]:getModule("Player"):GetUser(src)
    local price = 10000
    if user:getCash() >= price then --- MintHavoc Change Banking
        user:removeMoney(price) --- MintHavoc Change Banking
...
AddEventHandler('ragdoll:reviveSV', function(t)
    ...
    TriggerClientEvent('ucrp-hospital:client:RemoveBleed', t) 
    TriggerClientEvent('ucrp-hospital:client:ResetLimbs', t)
end)
